---
layout: post
title:  "BiscuitOS Linux newest riscv32 Usermanual"
date:   2019-06-06 14:45:30 +0800
categories: [Build]
excerpt: Linux newest riscv32 Usermanual.
tags:
  - Linux
---
...
riscv32 的开发部署，使用自动部署功能之后，BiscuitOS 项目会自动下载并部署 Linux 
newest riscv32 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者
参考下面命令进行部署:

{% highlight bash %}
cd BiscuitOS
make linux-newest-riscv32_defconfig
make
cd BiscuitOS/output/linux-newest-riscv32
{% endhighlight %}

成功执行之后，BiscuitOS 输出 Linux newest riscv32 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux newest riscv32 用户手册 (README)，开发者应该重点参考该 
README.md.
...
***********************************************
Output:
 BiscuitOS/output/linux-newest-

linux:
 BiscuitOS/output/linux-newest-riscv32/linux/linux 

README:
 BiscuitOS/output/linux-newest-riscv32/README.md 
...
{% highlight bash %}
cd BiscuitOS/output/linux-newest-riscv32/linux/linux
make ARCH=riscv clean
make ARCH=riscv BiscuitOS_riscv32_defconfig
{% endhighlight %}
...
{% highlight bash %}
cd BiscuitOS/output/linux-newest-riscv32/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
{% highlight bash %}
make CROSS_COMPILE=BiscuitOS/output/linux-5.0-riscv32/riscv32-biscuitos-linux-gnu/riscv32-biscuitos-linux-gnu/bin/riscv32-biscuitos-linux-gnu- -j4

make CROSS_COMPILE=BiscuitOS/output/linux-5.0-riscv32/riscv32-biscuitos-linux-gnu/riscv32-biscuitos-linux-gnu/bin/riscv32-biscuitos-linux-gnu- install
{% endhighlight %}
...
首先开发者应该准备基于 Linux newest riscv32 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-newest-riscv32/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
mkdir -p BiscuitOS/output/linux-newest-riscv32/linux/linux/drivers/BiscuitOS
{% endhighlight %}
...
{% highlight bash %}
make ARCH=riscv CROSS_COMPILE=BiscuitOS/output/linux-5.0-riscv32/riscv32-biscuitos-linux-gnu/riscv32-biscuitos-linux-gnu/bin/riscv32-biscuitos-linux-gnu- modules -j4
make ARCH=riscv INSTALL_MOD_PATH=BiscuitOS/output/linux-5.0-riscv32/rootfs/rootfs/ modules_install
{% endhighlight %}
...
首先开发者应该准备基于 Linux newest riscv32 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-newest-riscv32/package/Application_project-0.0.1
make prepare
make download
make
make install
...
{% highlight bash %}
BiscuitOS/output/linux-5.0-riscv32/riscv32-biscuitos-linux-gnu/riscv32-biscuitos-linux-gnu/bin/riscv32-biscuitos-linux-gnu-gdb BiscuitOS/output/linux-5.0-riscv32/linux/linux/vmlinux
{% endhighlight %}
