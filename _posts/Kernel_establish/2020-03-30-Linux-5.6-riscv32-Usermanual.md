---
layout: post
title:  "BiscuitOS Linux 5.6 riscv32 Usermanual"
date:   2019-06-06 14:45:30 +0800
categories: [Build]
excerpt: Linux 5.6 riscv32 Usermanual.
tags:
  - Linux
---
...
BiscuitOS 源码下载完毕后，使用 BiscuitOS 自动部署功能，部署一个 Linux 5.6 
riscv32 的开发部署，使用自动部署功能之后，BiscuitOS 项目会自动下载并部署 Linux 
5.6 riscv32 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者
参考下面命令进行部署:

{% highlight bash %}
cd BiscuitOS
make linux-5.6-riscv32_defconfig
make
cd BiscuitOS/output/linux-5.6-riscv32
{% endhighlight %}

成功执行之后，BiscuitOS 输出 Linux 5.6 riscv32 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux 5.6 riscv32 用户手册 (README)，开发者应该重点参考该 
README.md.
...
linux:
 BiscuitOS/output/linux-5.6-riscv32/linux/linux 

README:
 BiscuitOS/output/linux-5.6-riscv32/README.md 
...
{% highlight bash %}
cd BiscuitOS/output/linux-5.6-riscv32/linux/linux
...
{% highlight bash %}
cd BiscuitOS/output/linux-5.6-riscv32/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
首先开发者应该准备基于 Linux 5.6 riscv32 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-5.6-riscv32/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
mkdir -p BiscuitOS/output/linux-5.6-riscv32/linux/linux/drivers/BiscuitOS
{% endhighlight %}
...
首先开发者应该准备基于 Linux 5.6 riscv32 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-5.6-riscv32/package/Application_project-0.0.1
make prepare
make download
make
make install
