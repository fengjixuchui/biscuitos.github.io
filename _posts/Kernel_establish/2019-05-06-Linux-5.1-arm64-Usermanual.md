---
layout: post
title:  "Linux 5.1 arm64 Usermanual"
date:   2019-05-06 15:14:30 +0800
categories: [Build]
excerpt: Linux 5.1 arm64 Usermanual.
tags:
  - Linux
---
...
>
> - [配置 Linux 5.1 arm64 内核](#配置 Linux 5.1 arm64 内核)
>
> - [编译 Linux 5.1 arm64 内核](#编译 Linux 5.1 arm64 内核)
>
> - [运行 Linux 5.1 arm64 内核](#运行 Linux 5.1 arm64 内核)
>
> - [驱动开发](#驱动开发)
>
> - [调试 Linux 5.1 arm64 内核](#Linux 5.1 arm64 内核调试)
>
> - [附录](#附录)
...
以获得最新的 Linux 4.20, Linux 5.1 等等。只需要执行简单的命令，就能构建一个可
运行可调式的 Linux 开发环境。
...
获得 BiscuitOS 项目之后，可以使用 BiscuitOS 构建 Linux 5.1 arm64 的开发环境。开发者
只需执行如下命令就可以获得 Linux 5.1 arm64 完整的 BiscuitOS，如下：

{% highlight bash %}
cd BiscuitOS
make linux-5.1-arm64_defconfig
make
{% endhighlight %}
...
***********************************************
Output:
 BiscuitOS/output/linux-5.1-aarch

linux:
 BiscuitOS/output/linux-5.1-aarch/linux/linux

README:
 BiscuitOS/output/linux-5.1-aarch/README.md
...
```
cd BiscuitOS/output/linux-5.1-aarch/linux/linux
make ARCH=arm clean
...
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.1-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- -j8
```

# Build Busybox

```
cd BiscuitOS/output/linux-5.1-aarch/busybox/busybox
make clean
make menuconfig
  Busybox Settings --->
    Build Options --->
...
make CROSS_COMPILE=BiscuitOS/output/linux-5.1-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- -j8

make CROSS_COMPILE=BiscuitOS/output/linux-5.1-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- install
```
...
```
cd BiscuitOS/output/linux-5.1-aarch
...
```
BiscuitOS/output/linux-5.1-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu-gdb BiscuitOS/output/linux-5.1-aarch/linux/linux/vmlinux
...
#### <span id="配置 Linux 5.1 arm64 内核">配置 Linux 5.1 arm64 内核</span>
...
#### <span id="编译 Linux 5.1 arm64 内核">编译 Linux 5.1 arm64 内核</span>
...
#### <span id="运行 Linux 5.1 arm64 内核">运行 Linux 5.1 arm64 内核</span>

完成上面的步骤之后，开发者就可以运行 Linux 5.1 arm64 内核，使用如下命令即可：
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000349.png)

至此，一个 Linux 5.1 arm64 内核已经运行，开发者可以根据自己兴趣和需求对内核进行魔改。
...
持 Linux 5.1 arm64 内核驱动的开发。开发者可以参照下面方法进行更多驱动开发。
...
# <span id="Linux 5.1 arm64 内核调试">Linux 5.1 arm64 内核调试</span>
