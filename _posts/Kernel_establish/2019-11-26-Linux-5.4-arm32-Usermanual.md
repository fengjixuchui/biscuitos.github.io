---
layout: post
title:  "Linux 5.4 arm32 Usermanual"
date:   2019-05-06 14:45.40 +0800
categories: [Build]
excerpt: Linux 5.4 arm32 Usermanual.
tags:
  - Linux
---
...
至此，BiscuitOS 已经部署完毕.
BiscuitOS 目前已经支持 5.4 arm32 的开发部署，开发者在
部署完 BiscuitOS 环境之后，可以参考下面命令进行部署:

{% highlight bash %}
cd BiscuitOS
make linux-5.4-arm32_defconfig
make
cd BiscuitOS/output/linux-5.4-arm32
{% endhighlight %}

执行上面的命令之后，BiscuitOS 项目就会自动部署 "linux 5.4 arm32"
...
linux:
 BiscuitOS/output/linux-5.4-arm32/linux/linux 

README:
 BiscuitOS/output/linux-5.4-arm32/README.md 

***********************************************
{% endhighlight %}

在上面的输出信息中，指出了 "linux 5.4 arm32" 项目
...
要确定 "linux 5.4 arm32" 项目项目是否支持 bootloader 源码
编译，请参考 "[BiscuitOS 部署](#A11)" 一节中自动生成的 "README.md"
文档。如果文档中有关于 "Build Uboot" 章节，那么 "linux 5.4 arm32"
...
开发者首先参考 "linux 5.4 arm32" 项目生成的 README.md 文档，
...
参考 "linux 5.4 arm32" 项目生成的 README.md 文档，例如:

{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.4-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi-
{% endhighlight %}
...
工作，可以参考 "linux 5.4 arm32" 项目生成的 README.md 文档，
例如:
...
{% highlight bash %}
cd BiscuitOS/output/linux-5.4-arm32/linux/linux
make ARCH=arm clean
...
{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.4-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- -j8
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.4-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- dtbs
{% endhighlight %}
...
{% highlight bash %}
cd BiscuitOS/output/linux-5.4-arm32/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
{% highlight bash %}
make CROSS_COMPILE=BiscuitOS/output/linux-5.4-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- -j8

make CROSS_COMPILE=BiscuitOS/output/linux-5.4-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- install
{% endhighlight %}
...
![](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/RPI/Linux-5.4-arm32.png)
...
首先开发者应该准备基于 Linux 5.4 arm32 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-5.4-arm32/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.4-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- modules -j4
make ARCH=arm INSTALL_MOD_PATH=BiscuitOS/output/linux-5.4-arm32/rootfs/rootfs/ modules_install
{% endhighlight %}
...
{% highlight bash %}
BiscuitOS/output/linux-5.4-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi-gdb BiscuitOS/output/linux-5.4-arm32/linux/linux/vmlinux
{% endhighlight %}
