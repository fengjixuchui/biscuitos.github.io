---
layout: post
title:  "BiscuitOS Linux 5.x i386 Usermanual"
date:   2019-05-06 14:45:30 +0800
categories: [Build]
excerpt: Linux 5.x i386 Usermanual.
tags:
  - Linux
---
...
{% highlight bash %}
cd BiscuitOS
make linux-5.x-i386_defconfig
make
cd BiscuitOS/output/linux-5.x-i386
{% endhighlight %}
...
linux:
 BiscuitOS/output/linux-5.x-i386/linux/linux 

README:
 BiscuitOS/output/linux-5.x-i386/README.md 
...
{% highlight bash %}
cd BiscuitOS/output/linux-5.x-i386/linux/linux
...
{% highlight bash %}
cd BiscuitOS/output/linux-5.x-i386/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
首先开发者应该准备基于 Linux 5.x i386 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-5.x-i386/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
mkdir -p BiscuitOS/output/linux-5.x-i386/linux/linux/drivers/BiscuitOS
{% endhighlight %}
...
{% highlight bash %}
make ARCH=i386 modules -j4
make ARCH=i386 INSTALL_MOD_PATH=BiscuitOS/output/linux-5.x-i386/rootfs/rootfs/ modules_install
{% endhighlight %}
...
首先开发者应该准备基于 Linux 5.x i386 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-5.x-i386/package/Application_project-0.0.1
make prepare
make download
make
make install
...
{% highlight bash %}
gdb BiscuitOS/output/linux-5.x-i386/linux/linux/vmlinux
{% endhighlight %}
