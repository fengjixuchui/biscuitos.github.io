...
>
> - [配置 Linux 5.x arm64 内核](#配置 Linux 5.x arm64 内核)
>
> - [编译 Linux 5.x arm64 内核](#编译 Linux 5.x arm64 内核)
>
> - [运行 Linux 5.x arm64 内核](#运行 Linux 5.x arm64 内核)
>
> - [驱动开发](#驱动开发)
>
> - [调试 Linux 5.x arm64 内核](#Linux 5.x arm64 内核调试)
>
> - [附录](#附录)
...
以获得最新的 Linux 4.20, Linux 5.x 等等。只需要执行简单的命令，就能构建一个可
运行可调式的 Linux 开发环境。
...
获得 BiscuitOS 项目之后，可以使用 BiscuitOS 构建 Linux 5.x arm64 的开发环境。开发者
只需执行如下命令就可以获得 Linux 5.x arm64 完整的 BiscuitOS，如下：
...
***********************************************
Output:
 BiscuitOS/output/linux-5.x-aarch

linux:
 BiscuitOS/output/linux-5.x-aarch/linux/linux

README:
 BiscuitOS/output/linux-5.x-aarch/README.md
...
```
cd BiscuitOS/output/linux-5.x-aarch/linux/linux
make ARCH=arm clean
...
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.x-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- -j8
```

# Build Busybox

```
cd BiscuitOS/output/linux-5.x-aarch/busybox/busybox
make clean
make menuconfig
  Busybox Settings --->
    Build Options --->
...
make CROSS_COMPILE=BiscuitOS/output/linux-5.x-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- -j8

make CROSS_COMPILE=BiscuitOS/output/linux-5.x-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- install
```
...
```
cd BiscuitOS/output/linux-5.x-aarch
...
```
BiscuitOS/output/linux-5.x-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu-gdb BiscuitOS/output/linux-5.x-aarch/linux/linux/vmlinux
...
#### <span id="配置 Linux 5.x arm64 内核">配置 Linux 5.x arm64 内核</span>
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000343.png)

选择 **General setup**

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000344.png)

选中 **Initial RAM filesystem and RAM disk (initramfs/initrd) support**

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000345.png)

选择 **Device Driver**

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000346.png)

选择 **Block devices**

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000347.png)

选中 **RAM block device support**, 并设置 **Default RAM disk size** 的大小为
153600 . 保存并退出，至此，Linux 配置完成。

#### <span id="编译 Linux 5.x arm64 内核">编译 Linux 5.x arm64 内核</span>

配置完内核之后，接下来就是编译内核，根据 README 里提供的命令进行编译，具体命
令如下：

{% highlight bash %}
make ARCH=arm64 CROSS_COMPILE=BiscuitOS/output/linux-5.x-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- -j8
{% endhighlight %}

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000348.png)
...
#### <span id="运行 Linux 5.x arm64 内核">运行 Linux 5.x arm64 内核</span>

完成上面的步骤之后，开发者就可以运行 Linux 5.x arm64 内核，使用如下命令即可：
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000361.png)

至此，一个 Linux 5.x arm64 内核已经运行，开发者可以根据自己兴趣和需求对内核进行魔改。
...
持 Linux 5.x arm64 内核驱动的开发。开发者可以参照下面方法进行更多驱动开发。
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000350.png)

首先在目录中找到 **Device Driver --->** 回车并进入其中。

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000351.png)

接着在目录中找到 **BiscuitOS Driver --->** 按 Y 选中并按回车键进入。

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000352.png)
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000353.png)
...
# <span id="Linux 5.x arm64 内核调试">Linux 5.x arm64 内核调试</span>
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000354.png)
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000355.png)
