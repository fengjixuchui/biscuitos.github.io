---
layout: post
title:  "BiscuitOS Linux 2.6.34 - 2.6.39 arm32 Usermanual"
...
BiscuitOS 项目是一个用于制作 Linux 0.x、1.x、2.6.x、3.x、4.x、2.6.x
...
BiscuitOS 源码下载完毕后，使用 BiscuitOS 自动部署功能，部署一个 Linux 2.6.x 
arm32 的开发部署，使用自动部署功能之后，BiscuitOS 项目会自动下载并部署 Linux 
2.6.x arm32 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者
参考下面命令进行部署:
...
成功执行之后，BiscuitOS 输出 Linux 2.6.x arm32 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux 2.6.x arm32 用户手册 (README)，开发者应该重点参考该 
README.md.
...
{% highlight bash %}
cd BiscuitOS/output/linux-2.6.x-arm32/linux/linux
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-2.6.x-arm32/arm-none-linux-gnueabi/arm-none-linux-gnueabi/bin/arm-none-linux-gnueabi- -j4
{% endhighlight %}
...
{% highlight bash %}
make CROSS_COMPILE=BiscuitOS/output/linux-2.6.x-arm32/arm-none-linux-gnueabi/arm-none-linux-gnueabi/bin/arm-none-linux-gnueabi- -j4

make CROSS_COMPILE=BiscuitOS/output/linux-2.6.x-arm32/arm-none-linux-gnueabi/arm-none-linux-gnueabi/bin/arm-none-linux-gnueabi- install
{% endhighlight %}
...
首先开发者应该准备基于 Linux 2.6.x arm32 开发环境，然后
...
{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-2.6.x-arm32/arm-none-linux-gnueabi/arm-none-linux-gnueabi/bin/arm-none-linux-gnueabi- modules -j4
...
首先开发者应该准备基于 Linux 2.6.x arm32 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
BiscuitOS/output/linux-2.6.x-arm32/arm-none-linux-gnueabi/arm-none-linux-gnueabi/bin/arm-none-linux-gnueabi-gdb BiscuitOS/output/linux-2.6.x-arm32/linux/linux/vmlinux
{% endhighlight %}
