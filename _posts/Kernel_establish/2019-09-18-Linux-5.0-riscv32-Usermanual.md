---
layout: post
title:  "BiscuitOS Linux 5.0 riscv32 Usermanual"
...
5.0 riscv32 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者
参考下面命令进行部署:
...
成功执行之后，BiscuitOS 输出 Linux 5.0 riscv32 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux 5.0 riscv32 用户手册 (README)，开发者应该重点参考该 
README.md.
...
{% highlight bash %}
mkdir -p BiscuitOS/output/linux-5.0-riscv32/linux/linux/drivers/BiscuitOS
{% endhighlight %}
...
首先开发者应该准备基于 Linux 5.0 riscv32 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-5.0-riscv32/package/Application_project-0.0.1
make prepare
make download
make
make install
