---
layout: post
title:  "Linux 5.6 arm64 Usermanual"
date:   2020-03-30 14:45:30 +0800
categories: [Build]
excerpt: Linux 5.6 arm64 Usermanual.
tags:
  - Linux
---
...
至此，BiscuitOS 已经部署完毕.
BiscuitOS 目前已经支持 5.6 arm64 的开发部署，开发者在
部署完 BiscuitOS 环境之后，可以参考下面命令进行部署:

{% highlight bash %}
cd BiscuitOS
make linux-5.6-arm64_defconfig
make
cd BiscuitOS/output/linux-5.6-arm64
{% endhighlight %}

执行上面的命令之后，BiscuitOS 项目就会自动部署 "linux 5.6 arm64"
...
linux:
 BiscuitOS/output/linux-5.6-arm64/linux/linux 

README:
 BiscuitOS/output/linux-5.6-arm64/README.md 

***********************************************
{% endhighlight %}

在上面的输出信息中，指出了 "linux 5.6 arm64" 项目
...
{% highlight bash %}
cd BiscuitOS/output/linux-5.6-arm64/linux/linux
make ARCH=arm clean
...
{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.6-arm64/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- -j8
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.6-arm64/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- dtbs
{% endhighlight %}
...
{% highlight bash %}
cd BiscuitOS/output/linux-5.6-arm64/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
{% highlight bash %}
make CROSS_COMPILE=BiscuitOS/output/linux-5.6-arm64/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- -j8

make CROSS_COMPILE=BiscuitOS/output/linux-5.6-arm64/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- install
{% endhighlight %}
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000600.png)
...
首先开发者应该准备基于 Linux 5.6 arm64 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-5.6-arm64/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.6-arm64/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- modules -j4
make ARCH=arm INSTALL_MOD_PATH=BiscuitOS/output/linux-5.6-arm64/rootfs/rootfs/ modules_install
{% endhighlight %}
...
{% highlight bash %}
BiscuitOS/output/linux-5.6-arm64/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu-gdb BiscuitOS/output/linux-5.6-arm64/linux/linux/vmlinux
{% endhighlight %}
