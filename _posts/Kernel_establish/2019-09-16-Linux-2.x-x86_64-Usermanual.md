---
layout: post
title:  "BiscuitOS Linux 2.x x86_64 Usermanual"
date:   2019-05-06 14:45:30 +0800
categories: [Build]
excerpt: Linux 2.x x86_64 Usermanual.
tags:
  - Linux
---
...
BiscuitOS 项目是一个用于制作 Linux 0.x、1.x、2.x、3.x、2.x、2.x
...
BiscuitOS 源码下载完毕后，使用 BiscuitOS 自动部署功能，部署一个 Linux 2.x 
x86_64 的开发部署，使用自动部署功能之后，BiscuitOS 项目会自动下载并部署 Linux 
2.x x86_64 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者首先查看 BiscuitOS 支持的 2.x x86_64 版本，使用如下命令进行查询:

{% highlight bash %}
cd BiscuitOS
ls configs/ | grep linux-2.*-x86_64_defconfig
{% endhighlight %}
...
{% highlight bash %}
cd BiscuitOS
make linux-2.x-x86_64_defconfig
make
cd BiscuitOS/output/linux-2.x-x86_64
{% endhighlight %}

成功执行之后，BiscuitOS 输出 Linux 2.x x86_64 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux 2.x x86_64 用户手册 (README)，开发者应该重点参考该 
README.md.
...
***********************************************
Output:
 BiscuitOS/output/linux-2.x-

linux:
 BiscuitOS/output/linux-2.x-x86_64/linux/linux 

README:
 BiscuitOS/output/linux-2.x-x86_64/README.md 
...
由于 Ubuntu 16.04/18.04/20.04 等使用高版本的 GCC，但低版本的内核需要使用低版本的 GCC 进行编译，因此在编译内核的时候需要安装低版本的 GCC，但开发者一定要注意低版本的 GCC 只能用于内核和 busybox 相关的编译，例如项目部署的时候只能使用原版本的 GCC，开发者可以参考如下命令进行安装，首先修改 Ubuntu 的 APT 源，使其能支持低版本的 GCC，本例子以 Ubuntu 18.04 进行讲解.
...
接着使用如下命令在当前系统中兼容多个版本的 GCC，使用如下命令, 如果是第一次支持多版本的 GCC，请执行如下命令，如果已经支持，请跳过这个命令:
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/HK/HK000491.png)
...
{% highlight bash %}
cd BiscuitOS/output/linux-2.x-x86_64/linux/linux
...
{% highlight bash %}
cd BiscuitOS/output/linux-2.x-x86_64/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
"-m32 -march=x86_64 -mtune=x86_64" 到 Additional CFLAGS, 添加 "-m32" 
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/HK/HK000497.png)
...
首先开发者应该准备基于 Linux 2.x x86_64 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-2.x-x86_64/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
mkdir -p BiscuitOS/output/linux-2.x-x86_64/linux/linux/drivers/BiscuitOS
{% endhighlight %}
...
{% highlight bash %}
make ARCH=x86_64 modules -j4
make ARCH=x86_64 INSTALL_MOD_PATH=BiscuitOS/output/linux-2.x-x86_64/rootfs/rootfs/ modules_install
{% endhighlight %}
...
{% highlight bash %}
cd lib/modules/2.x.0/kernel/driver/BiscuitOS/
insmod misc.ko
{% endhighlight %}
...
首先开发者应该准备基于 Linux 2.x x86_64 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-2.x-x86_64/package/Application_project-0.0.1
make prepare
make download
make
make install
...
{% highlight bash %}
gdb BiscuitOS/output/linux-2.x-x86_64/linux/linux/vmlinux
{% endhighlight %}
