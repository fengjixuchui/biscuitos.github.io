---
layout: post
title:  "BiscuitOS Linux 4.x i386 Usermanual"
date:   2019-05-06 14:45:30 +0800
categories: [Build]
excerpt: Linux 4.x i386 Usermanual.
tags:
  - Linux
---
...
BiscuitOS 项目是一个用于制作 Linux 0.x、1.x、2.x、3.x、4.x、4.x
...
4.x i386 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者首先查看 BiscuitOS 支持的 4.x i386 版本，使用如下命令进行查询:
...
{% highlight bash %}
cd BiscuitOS
make linux-4.x-i386_defconfig
make
cd BiscuitOS/output/linux-4.x-i386
{% endhighlight %}

成功执行之后，BiscuitOS 输出 Linux 4.x i386 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux 4.x i386 用户手册 (README)，开发者应该重点参考该 
README.md.
...
***********************************************
Output:
 BiscuitOS/output/linux-4.x-

linux:
 BiscuitOS/output/linux-4.x-i386/linux/linux 

README:
 BiscuitOS/output/linux-4.x-i386/README.md 
...
{% highlight bash %}
cd BiscuitOS/output/linux-4.x-i386/linux/linux
...
{% highlight bash %}
cd BiscuitOS/output/linux-4.x-i386/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/HK/HK000493.png)
...
首先开发者应该准备基于 Linux 4.x i386 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-4.x-i386/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
mkdir -p BiscuitOS/output/linux-4.x-i386/linux/linux/drivers/BiscuitOS
{% endhighlight %}
...
{% highlight bash %}
make ARCH=i386 modules -j4
make ARCH=i386 INSTALL_MOD_PATH=BiscuitOS/output/linux-4.x-i386/rootfs/rootfs/ modules_install
{% endhighlight %}
...
首先开发者应该准备基于 Linux 4.x i386 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-4.x-i386/package/Application_project-0.0.1
make prepare
make download
make
make install
...
{% highlight bash %}
gdb BiscuitOS/output/linux-4.x-i386/linux/linux/vmlinux
{% endhighlight %}
