---
layout: post
title:  "BiscuitOS Linux 0.97 i386 Usermanual"
date:   2020-10-02 14:45:30 +0800
categories: [Build]
excerpt: Linux 0.97 i386 Usermanual.
tags:
  - Linux
---
...
BiscuitOS 项目是一个用于制作 Linux 0.x、1.x、2.x、3.x、4.x、0.97
...
BiscuitOS 源码下载完毕后，使用 BiscuitOS 自动部署功能，部署一个 Linux 0.97 
i386 的开发部署，使用自动部署功能之后，BiscuitOS 项目会自动下载并部署 Linux 
0.97 i386 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者
参考下面命令进行部署:
...
make
cd BiscuitOS/output/linux-0.97
{% endhighlight %}

成功执行之后，BiscuitOS 输出 Linux 0.97 i386 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux 0.97 i386 用户手册 (README)，开发者应该重点参考该 
README.md.
...
***********************************************
Output:
 BiscuitOS/output/linux-0.97-

linux:
 BiscuitOS/output/linux-0.97/linux/linux 

README:
 BiscuitOS/output/linux-0.97/README.md 
...
{% highlight bash %}
cd BiscuitOS/output/linux-0.97/linux/linux
make clean
make menuconfig
{% endhighlight %}
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/BiscuitOS/kernel/BUDX000047.png)
...
可以使用 GDB 对内核进行单步调试。Linux 0.97 内核由于启动的分为不同阶段，所以每个
阶段有不同的调试方法，具体分为以下三种：
...
{% highlight bash %}
cd BiscuitOS/output/linux-0.97/linux/linux/arch/x86/boot/
gdb .debug_bootsect
...
Linux 0.97 在启动阶段会将自己从 0x7c00 拷贝到 0x90000 处继续运行，这个阶段的代码
...
Linux 0.97 内核执行完 bootsect.s 汇编代码之后，接下来执行的代码位于
...
Linux 0.97 在进行基本初始化之后，就进入保护模式，开启分页功能，这个阶段的也包含了
一部分汇编代码，位于 arch/x86/boot/head.s 文件里面。由 Linux 0.97 的原理可以
