...
在上面的输出信息中，指出了 "linux 5.x riscv32" 项目
...
要确定 "linux 5.x riscv32" 项目项目是否支持 bootloader 源码
编译，请参考 "[BiscuitOS 部署](#A11)" 一节中自动生成的 "README.md"
文档。如果文档中有关于 "Build Uboot" 章节，那么 "linux 5.x riscv32"
...
开发者首先参考 "linux 5.x riscv32" 项目生成的 README.md 文档，
...
参考 "linux 5.x riscv32" 项目生成的 README.md 文档，例如:

{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-5.x-riscv32/riscv32-biscuitos-linux-gnu/riscv32-biscuitos-linux-gnu/bin/riscv32-biscuitos-linux-gnu-
{% endhighlight %}
...
工作，可以参考 "linux 5.x riscv32" 项目生成的 README.md 文档，
例如:
...
![](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/RPI/Linux-5.x-riscv32.png)
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-5.x-riscv32/package/LDD_platform_core_module-0.0.1/
make prepare
make download
make
make install
