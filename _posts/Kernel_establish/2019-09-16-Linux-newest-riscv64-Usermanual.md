---
layout: post
title:  "BiscuitOS Linux newest riscv64 Usermanual"
date:   2019-06-06 14:45:30 +0800
categories: [Build]
excerpt: Linux newest riscv64 Usermanual.
tags:
  - Linux
---
...
newest riscv64 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者
参考下面命令进行部署:

{% highlight bash %}
cd BiscuitOS
make linux-newest-riscv64_defconfig
make
cd BiscuitOS/output/linux-newest-riscv64
{% endhighlight %}

成功执行之后，BiscuitOS 输出 Linux newest riscv64 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux newest riscv64 用户手册 (README)，开发者应该重点参考该 
README.md.
...
linux:
 BiscuitOS/output/linux-newest-riscv64/linux/linux 

README:
 BiscuitOS/output/linux-newest-riscv64/README.md 
...
{% highlight bash %}
cd BiscuitOS/output/linux-newest-riscv64/linux/linux
...
{% highlight bash %}
cd BiscuitOS/output/linux-newest-riscv64/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
首先开发者应该准备基于 Linux newest riscv64 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-newest-riscv64/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
mkdir -p BiscuitOS/output/linux-newest-riscv64/linux/linux/drivers/BiscuitOS
{% endhighlight %}
...
首先开发者应该准备基于 Linux newest riscv64 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-newest-riscv64/package/Application_project-0.0.1
make prepare
make download
make
make install
