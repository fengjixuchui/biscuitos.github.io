---
layout: post
title:  "BiscuitOS Linux 4.x arm32 Usermanual"
date:   2019-06-06 14:45:30 +0800
categories: [Build]
excerpt: Linux 4.x arm32 Usermanual.
tags:
  - Linux
---
...
BiscuitOS 项目是一个用于制作 Linux 0.x、1.x、4.x、4.x、4.x、4.x
...
BiscuitOS 源码下载完毕后，使用 BiscuitOS 自动部署功能，部署一个 Linux 4.x 
arm32 的开发部署，使用自动部署功能之后，BiscuitOS 项目会自动下载并部署 Linux 
4.x arm32 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者
参考下面命令进行部署:

{% highlight bash %}
cd BiscuitOS
make linux-4.x-arm32_defconfig
make
cd BiscuitOS/output/linux-4.x-arm32
{% endhighlight %}

成功执行之后，BiscuitOS 输出 Linux 4.x arm32 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux 4.x arm32 用户手册 (README)，开发者应该重点参考该 
README.md.
...
***********************************************
Output:
 BiscuitOS/output/linux-4.x-arm32

linux:
 BiscuitOS/output/linux-4.x-arm32/linux/linux 

README:
 BiscuitOS/output/linux-4.x-arm32/README.md 
...
{% highlight bash %}
cd BiscuitOS/output/linux-4.x-arm32/linux/linux
make ARCH=arm clean
...
{% highlight bash %}
cd BiscuitOS/output/linux-4.x-arm32/linux/linux
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-4.x-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- -j4
{% endhighlight %}
...
{% highlight bash %}
cd BiscuitOS/output/linux-4.x-arm32/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
{% highlight bash %}
make CROSS_COMPILE=BiscuitOS/output/linux-4.x-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- -j4

make CROSS_COMPILE=BiscuitOS/output/linux-4.x-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- install
{% endhighlight %}
...
首先开发者应该准备基于 Linux 4.x arm32 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-4.x-arm32/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
mkdir -p BiscuitOS/output/linux-4.x-arm32/linux/linux/drivers/BiscuitOS
{% endhighlight %}
...
{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-4.x-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- modules -j4
make ARCH=arm INSTALL_MOD_PATH=BiscuitOS/output/linux-4.x-arm32/rootfs/rootfs/ modules_install
{% endhighlight %}
...
{% highlight bash %}
cd lib/modules/4.x.0/kernel/driver/BiscuitOS/
insmod misc.ko
{% endhighlight %}
...
首先开发者应该准备基于 Linux 4.x arm32 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-4.x-arm32/package/Application_project-0.0.1
make prepare
make download
make
make install
...
{% highlight bash %}
BiscuitOS/output/linux-4.x-arm32/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi-gdb BiscuitOS/output/linux-4.x-arm32/linux/linux/vmlinux
{% endhighlight %}
