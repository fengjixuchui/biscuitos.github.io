---
layout: post
title:  "Linux 5.x riscv Usermanual"
date:   2019-05-06 14:45:30 +0800
categories: [Build]
excerpt: Linux 5.x riscv Usermanual.
tags:
  - Linux
---
...
> - Linux 5.x riscv 内核
>
>   - [配置 Linux 5.x riscv 内核](#配置 Linux 5.x riscv 内核)
>
>   - [编译 Linux 5.x riscv 内核](#编译 Linux 5.x riscv 内核)
>
>   - [运行 Linux 5.x riscv 内核](#运行 Linux 5.x riscv 内核)
>
>   - [驱动开发](#驱动开发)
>
> - [Linux 5.x riscv 调试](#Linux 5.x riscv 内核调试)
>
> - [Linux 5.x riscv 网络使用](https://biscuitos.github.io/blog/Networking-Usermanual/)
>
> - [Linux 5.x riscv 应用开发](https://biscuitos.github.io/blog/APP-Usermanual/)
>
> - [附录](#附录)
...
{% highlight bash %}
sudo apt-get install -y gcc make gdb git figlet
...
sudo apt-get install -y build-essential bison flex texinfo gperf 
sudo apt-get install -y curl libmpc-dev libmpfr-dev libgmp-dev zlib1g-dev 
sudo apt-get install -y libtool patchutils bc libexpat-dev gawk
{% endhighlight %}
...
获得 BiscuitOS 项目之后，可以使用 BiscuitOS 构建 Linux 5.x riscv 的开发环境。
BiscuitOS 支持 riscv64 与 RISCV64 内核开发，开发者可以根据需求，选择合适的版本
进行内核开发。开发者只需执行如下命令就可以获得 Linux 5.x riscv 完整的 BiscuitOS，如下：
...
binutils, GNU-GCC, riscv-GCC,Busybox 和 Qemu 等工具，以此构建一个完整的
...
***********************************************
Output:
 BiscuitOS/output/linux-5.x-riscv64
...
make ARCH=riscv CROSS_COMPILE=BiscuitOS/output/linux-5.x-riscv64/riscv64-biscuitos-linux-gnu/riscv64-biscuitos-linux-gnu/bin/riscv64-biscuitos-linux-gnu- vmlinux -j8
```
...
make CROSS_COMPILE=BiscuitOS/output/linux-5.x-riscv64/riscv64-biscuitos-linux-gnu/riscv64-biscuitos-linux-gnu/bin/riscv64-biscuitos-linux-gnu- -j8

make CROSS_COMPILE=BiscuitOS/output/linux-5.x-riscv64/riscv64-biscuitos-linux-gnu/riscv64-biscuitos-linux-gnu/bin/riscv64-biscuitos-linux-gnu- install
```
...
```
BiscuitOS/output/linux-5.x-riscv64/riscv64-biscuitos-linux-gnu/riscv64-biscuitos-linux-gnu/bin/riscv64-biscuitos-linux-gnu-gdb BiscuitOS/output/linux-5.x-riscv64/linux/linux/vmlinux
...
#### <span id="配置 Linux 5.x riscv 内核">配置 Linux 5.x riscv 内核</span>
...
#### <span id="编译 Linux 5.x riscv 内核">编译 Linux 5.x riscv 内核</span>
...
#### <span id="运行 Linux 5.x riscv 内核">运行 Linux 5.x riscv 内核</span>

完成上面的步骤之后，开发者就可以运行 Linux 5.x riscv 内核，使用如下命令即可：
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/mall/MALL000010.png)

至此，一个 Linux 5.x riscv 内核已经运行，开发者可以根据自己兴趣和需求对内核进行魔改。
...
持 Linux 5.x riscv 内核驱动的开发。开发者可以参照下面方法进行更多驱动开发。
...
{% highlight bash %}
make ARCH=riscv CROSS_COMPILE=BiscuitOS/output/linux-5.x-riscv64/riscv64-biscuitos-linux-gnu/riscv64-biscuitos-linux-gnu/bin/riscv64-biscuitos-linux-gnu- -j8
{% endhighlight %}
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000340.png)
...
# <span id="Linux 5.x riscv 内核调试">Linux 5.x riscv 内核调试</span>
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000341.png)
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000342.png)
