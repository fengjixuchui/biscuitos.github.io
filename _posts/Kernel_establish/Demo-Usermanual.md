---
layout: post
title:  "Linux PKVER PARCH Usermanual"
date:   2019-05-06 14:45:30 +0800
categories: [Build]
excerpt: Linux PKVER PARCH Usermanual.
tags:
  - Linux
---

> PARCH  ---> arm32/arm64/riscv32/riscv64
> PKVER  ---> kernel version
> DEFCONFIG ---> vexpress_defconfig
> RPI/Linux-PKVER-PARCH.png --> Running Picture
...
更多有趣的可能，开发者在使用 BiscuitOS 构建 Linux PKVER
的系统之前，需要做好如下准备:
...
至此，BiscuitOS 已经部署完毕.
BiscuitOS 目前已经支持 PKVER PARCH 的开发部署，开发者在
部署完 BiscuitOS 环境之后，可以参考下面命令进行部署:

{% highlight bash %}
cd BiscuitOS
make linux-PKVER-PARCH_defconfig
make
cd BiscuitOS/output/linux-PKVER-PARCH
{% endhighlight %}

执行上面的命令之后，BiscuitOS 项目就会自动部署 "linux PKVER PARCH"
...
***********************************************
Output:
 BiscuitOS/output/linux-PKVER-

linux:
 BiscuitOS/output/linux-PKVER-PARCH/linux/linux 

README:
 BiscuitOS/output/linux-PKVER-PARCH/README.md 

***********************************************
{% endhighlight %}

在上面的输出信息中，指出了 "linux PKVER PARCH" 项目
...
要确定 "linux PKVER PARCH" 项目项目是否支持 bootloader 源码
编译，请参考 "[BiscuitOS 部署](#A11)" 一节中自动生成的 "README.md"
文档。如果文档中有关于 "Build Uboot" 章节，那么 "linux PKVER PARCH"
...
开发者首先参考 "linux PKVER PARCH" 项目生成的 README.md 文档，
...
参考 "linux PKVER PARCH" 项目生成的 README.md 文档，例如:

{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-PKVER-PARCH/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi-
{% endhighlight %}
...
工作，可以参考 "linux PKVER PARCH" 项目生成的 README.md 文档，
例如:
...
{% highlight bash %}
cd BiscuitOS/output/linux-PKVER-PARCH/linux/linux
make ARCH=arm clean
...
{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-PKVER-PARCH/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- -j8
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-PKVER-PARCH/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- dtbs
{% endhighlight %}
...
{% highlight bash %}
cd BiscuitOS/output/linux-PKVER-PARCH/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
{% highlight bash %}
make CROSS_COMPILE=BiscuitOS/output/linux-PKVER-PARCH/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- -j8

make CROSS_COMPILE=BiscuitOS/output/linux-PKVER-PARCH/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- install
{% endhighlight %}
...
![](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/RPI/Linux-PKVER-PARCH.png)
...
首先开发者应该准备基于 Linux PKVER PARCH 开发环境，然后
...
设置 "LDD: platform Core Module --->" 为 "Y"。设置完毕之后，
保存并退出.
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-PKVER-PARCH/package/LDD_platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
cd lib/modules/PKVER.0/extra/
insmod LDD_platform_core_module-0.0.1
{% endhighlight %}
...
{% highlight bash %}
make ARCH=arm CROSS_COMPILE=BiscuitOS/output/linux-PKVER-PARCH/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi- modules -j4
make ARCH=arm INSTALL_MOD_PATH=BiscuitOS/output/linux-PKVER-PARCH/rootfs/rootfs/ modules_install
{% endhighlight %}
...
{% highlight bash %}
cd lib/modules/PKVER.0/kernel/driver/BiscuitOS/
insmod misc.ko
{% endhighlight %}
...
{% highlight bash %}
BiscuitOS/output/linux-PKVER-PARCH/arm-linux-gnueabi/arm-linux-gnueabi/bin/arm-linux-gnueabi-gdb BiscuitOS/output/linux-PKVER-PARCH/linux/linux/vmlinux
{% endhighlight %}
