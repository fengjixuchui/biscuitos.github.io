---
layout: post
title:  "BiscuitOS Linux newest i386 Usermanual"
date:   2019-05-06 14:45:30 +0800
categories: [Build]
excerpt: Linux newest i386 Usermanual.
tags:
  - Linux
---
...
newest i386 所需的源码和编译工具等，可能需要一些时间，请保持网络的通畅. 开发者
参考下面命令进行部署:

{% highlight bash %}
cd BiscuitOS
make linux-newest-i386_defconfig
make
cd BiscuitOS/output/linux-newest-i386
{% endhighlight %}

成功执行之后，BiscuitOS 输出 Linux newest i386 项目目录，以及对应内核的目录，并且
自动生成 BiscuitOS Linux newest i386 用户手册 (README)，开发者应该重点参考该 
README.md.
...
linux:
 BiscuitOS/output/linux-newest-i386/linux/linux 

README:
 BiscuitOS/output/linux-newest-i386/README.md 
...
{% highlight bash %}
cd BiscuitOS/output/linux-newest-i386/linux/linux
...
{% highlight bash %}
cd BiscuitOS/output/linux-newest-i386/busybox/busybox
make clean
make menuconfig
{% endhighlight %}
...
首先开发者应该准备基于 Linux newest i386 开发环境，然后
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-newest-i386/package/platform_core_module-0.0.1/
make prepare
make download
make
make install
...
{% highlight bash %}
mkdir -p BiscuitOS/output/linux-newest-i386/linux/linux/drivers/BiscuitOS
{% endhighlight %}
...
{% highlight bash %}
make ARCH=i386 modules -j4
make ARCH=i386 INSTALL_MOD_PATH=BiscuitOS/output/linux-newest-i386/rootfs/rootfs/ modules_install
{% endhighlight %}
...
首先开发者应该准备基于 Linux newest i386 开发环境，然后使用如下命令获得应用程序所
需的开发环境:
...
{% highlight bash %}
make
cd BiscuitOS/output/linux-newest-i386/package/Application_project-0.0.1
make prepare
make download
make
make install
...
{% highlight bash %}
gdb BiscuitOS/output/linux-newest-i386/linux/linux/vmlinux
{% endhighlight %}
