...
对于四个参数系统调用的返回值，返回的数据类型与传入参数无关，因此开发者
可以根据需求自行定义返回的数据。
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000453.png)

选择 "[\*]   SYSCALL_DEFINE4(): Four Paramenter  --->" 保存配置并退出. 
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000454.png)

上图中，main.c 与用户空间系统调用相关的源码,
"SYSCALL_DEFINE4_common-0.0.1/Makefile" 是 main.c 交叉编译的逻辑。
"SYSCALL_DEFINE4_common-0.0.1/BiscuitOS_syscall.c" 文件是新系统调用
内核实现。因此对于用户空间的系统调用，开发者只需关注 main.c, 内容如下:

![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000455.png)
...
一个整形变量 nr_read, 以及一个整形指针通过系统调用传递到内核。如果系
统调用执行成功，buffer 将接受从内核传递的字符串，并打印该字符串。源码
准备好之后，接下来是交叉编译源码并打包到 rootfs 里，最后在 ARM32 上运行。
使用如下命令:
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000456.png)

新系统调用的内核实现逻辑通过调用 "SYSCALL_DEFINE4()" 宏实现,第一个参数
...
整形数据。内核定义了一个字符数组，长度为 128，也定义了一个字
...
拷贝到字符串的结尾字节。同理将 Kreader 变量传递到用户空间。准备好源码之后，
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000457.png)
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000458.png)
