...
对于一个参数系统调用的返回值，返回的数据类型与传入参数无关，因此开发者
可以根据需求自行定义返回的数据。
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000434.png)
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000435.png)

上图中，main.c 与用户空间系统调用相关的源码,
"SYSCALL_DEFINE1_common-0.0.1/Makefile" 是 main.c 交叉编译的逻辑。
"SYSCALL_DEFINE1_common-0.0.1/BiscuitOS_syscall.c" 文件是新系统调用
内核实现。因此对于用户空间的系统调用，开发者只需关注 main.c, 内容如下:

![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000436.png)

根据在内核中创建的入口，这里定义了入口宏的值为 400，一定要与内核定义
的入口值相呼应. 在上图的程序中，定义了一个 char 的数组，数组长度为
128，初始值为 "BiscuitOS_Userspace", 通过调用 "syscall()" 函数，将
系统调用号 \_\_NR_hello_BiscuitOS 和 buffer 通过系统调用传递到内核。
如果系统调用执行成功，buffer 将接受从内核传递的字符串，并打印该字符串。
...
{% highlight bash %}
cp -rfa BiscuitOS/output/linux-5.0-arm32/package/SYSCALL_DEFINE1_common-0.0.1/BiscuitOS_syscall.c  BiscuitOS/output/linux-5.0-arm32/linux/linux/fs/
cd BiscuitOS/output/linux-5.0-arm32/linux/linux/fs
vi BiscuitOS_syscall.c
{% endhighlight %}

![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000437.png)

新系统调用的内核实现逻辑通过调用 "SYSCALL_DEFINE1()" 宏实现，其
定义了一个来自用户空间的 char 指针参数，参数的名字为 strings. 函数
定义了一个字符数组，长度为 128，也定义了一个字符常量 kstring, 其值为
"BiscuitOS_kernel". 函数通过调用 copy_from_user() 函数从用户空间拷贝
字符串到内核空间，并存储到 buffer 字符数组里，拷贝长度为 128. 如果拷贝
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000438.png)
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000439.png)
