...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000465.png)

选择 "[\*]   SYSCALL_DEFINE6(): Six Paramenter  --->" 保存配置并退出. 
...
{% highlight bash %}
cd BiscuitOS/output/linux-5.0-arm32/package/SYSCALL_DEFINE6_common-0.0.1
{% endhighlight %}
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000466.png)

上图中，main.c 与用户空间系统调用相关的源码,
"SYSCALL_DEFINE6_common-0.0.1/Makefile" 是 main.c 交叉编译的逻辑。
"SYSCALL_DEFINE6_common-0.0.1/BiscuitOS_syscall.c" 文件是新系统调用
内核实现。因此对于用户空间的系统调用，开发者只需关注 main.c, 内容如下:

![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000467.png)
...
一个整形变量 nr_read、一个整形指针、一个结构体，以及一个整形数组通过系
统调用传递到内核。如果系统调用执行成功，buffer 将接受从内核传递的字符
串，并打印该字符串。源码准备好之后，接下来是交叉编译源码并打包到 
rootfs 里，最后在ARM32 上运行。使用如下命令:
...
{% highlight bash %}
cp -rfa BiscuitOS/output/linux-5.0-arm32/package/SYSCALL_DEFINE6_common-0.0.1/BiscuitOS_syscall.c  BiscuitOS/output/linux-5.0-arm32/linux/linux/fs/
cd BiscuitOS/output/linux-5.0-arm32/linux/linux/fs
vi BiscuitOS_syscall.c
{% endhighlight %}

![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000468.png)

新系统调用的内核实现逻辑通过调用 "SYSCALL_DEFINE6()" 宏实现，第一个参数
...
整形数据。第五个参数是一个结构体，第六个参数是一个整形数组，其定义如上图。
内核定义了一个字符数组，长度为 128，也定义了一个字符常量 kstring, 其值
为 "BiscuitOS_kernel". 函数通过调用 copy_from_user() 函数从用户空间拷贝
字符串到内核空间，并存储到 buffer 字符数组里，拷贝长度为 nr. 如果拷贝成
功，则打印该字符串。接着调用 copy_to_user() 函数，将内核的字符
串 "BiscuitOS_kernel" 拷贝到内核空间，拷贝长度为 nr_read，这样确保拷贝
到字符串的结尾字节。同理将 Kreader 变量传递到用户空间, 最后内核也使
用 copy_to_user() 函数将一个结构体的数据拷贝到了用户空间。对于传递整形
数组，同样调用 copy_to_user() 函数完成。准备好源码之后，接着修改内核源
...
![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000469.png)
...
{% highlight bash %}
~ #
~ # strace SYSCALL_DEFINE6_common-0.0.1
{% endhighlight %}

![](https://gitee.com/BiscuitOS_team/PictureSet/raw/Gitee/RPI/RPI000470.png)
