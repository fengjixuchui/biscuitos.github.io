---
layout: post
title:  "memblock_phys_alloc_try_nid() 从指定 NUMA 节点中分配指定物理内存"
date:   2019-03-13 14:45:30 +0800
categories: [MMU]
excerpt: memblock_phys_alloc_try_nid() 从指定 NUMA 节点中分配指定物理内存.
tags:
  - MMU
---

> [GitHub: MEMBLOCK memblock_phys_alloc_try_nid()](https://github.com/BiscuitOS/HardStack/tree/master/Memory-Allocator/Memblock-allocator/API/memblock_phys_alloc_try_nid)
>
...
函数： memblock_phys_alloc_try_nid()

功能： 从指定 NUMA 节点中分配物理内存
...
参数 size 代表要分配区块的大小，align 表示分配区块的对齐大小；nid 代表 NUMA 号。

函数首先调用 memblock_phys_alloc_nid() 函数去分配物理内存，如果分配失败，那么调用
memblock_alloc_base() 函数分配，这样分配不容易失败。
...
函数直接调用 memblock_alloc_base_nid() 函数，分配一块可用的物理内存，并将
这块内存区加入预留区内。最后对 flags 参数和返回的结果进行检测，如果 ret 为 0，
并且 flags 包含 MEMBLOCK_MIRROR, 那么修改 flags 属性，去掉 MEMBLOCK_MIRROR,
然后重新分配。
...
memblock_alloc_base() 函数源码分析请看：[memblock_alloc_base() 源码](https://biscuitos.github.io/blog/MMU-ARM32-MEMBLOCK-memblock_alloc_base/#源码分析)
...
memblock_phys_alloc_try_nid() 函数用于从指定的 NUMA 节点中分配物理内存，
实践目的是通过这个函数分配指定大小的物理内存。
...
#ifdef CONFIG_DEBUG_MEMBLOCK_PHYS_ALLOC_TRY_NID
int __init debug_memblock_phys_alloc_try_nid(void)
{
	 ...
	 */
	addr = memblock_phys_alloc_try_nid(0x100000, 0x1000, NUMA_NO_NODE);
...
+#ifdef CONFIG_DEBUG_MEMBLOCK_PHYS_ALLOC_TRY_NID
+	extern int bs_debug;
+	extern int debug_memblock_phys_alloc_try_nid(void);
+#endif
...
ck_add
+#ifdef CONFIG_DEBUG_MEMBLOCK_PHYS_ALLOC_TRY_NID
+	debug_memblock_phys_alloc_try_nid();
+#endif
+
 	early_fixmap_init();
 	early_ioremap_init();
...
memblock_phys_alloc_try_nid() 函数分配一个大小为 0x100000 的内存区块，
代码如下：
...
从运行的结果可以知道，memblock_phys_alloc_try_nid() 函数找到了一块起始地址为
