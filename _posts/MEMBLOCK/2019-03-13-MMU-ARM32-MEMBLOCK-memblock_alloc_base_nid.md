---
layout: post
title:  "memblock_alloc_base_nid() 从 0 地址开始分配一块可用的内存"
date:   2019-03-13 14:27:30 +0800
categories: [MMU]
excerpt: memblock_alloc_base_nid() 从 0 地址开始分配一块可用的内存.
tags:
  - MMU
---

> [GitHub: MEMBLOCK memblock_alloc_base_nid()](https://github.com/BiscuitOS/HardStack/tree/master/Memory-Allocator/Memblock-allocator/API/memblock_alloc_base_nid)
>
...
函数： memblock_alloc_base_nid()

功能： 从 0 地址开发分配一块可用的物理内存区块
...
memblock_alloc_base_nid() 函数用于从指定的内存区域内分配一块可用的物理内存。
本次实践的目的就是介绍如何使用该函数进行内存分配。
...
#ifdef CONFIG_DEBUG_MEMBLOCK_ALLOC_BASE_NID
int __init debug_memblock_alloc_base_nid(void)
{
	 ...
	 */
	addr = memblock_alloc_base_nid(0x100000, 0x1000,
			 ARCH_LOW_ADDRESS_LIMIT, NUMA_NO_NODE, flags);
...
+
+config DEBUG_MEMBLOCK_ALLOC_BASE_NID
+       bool "memblock_alloc_base_nid()"
+
...
+#ifdef CONFIG_DEBUG_MEMBLOCK_ALLOC_BASE_NID
+	extern int bs_debug;
+	extern int debug_memblock_alloc_base_nid(void);
+#endif
...
memblock_alloc_base_nid() 函数分配一个大小为 0x100000 的内存区块，代码如下：
...
从运行的结果可以知道，memblock_alloc_base_nid() 函数找到了一块起始地址为
