---
layout: post
title:  "memblock_alloc_range() 从指定范围内分配指定大小的内存区块"
date:   2019-03-13 12:04:30 +0800
categories: [MMU]
excerpt: memblock_alloc_range() 从指定范围内分配指定大小的内存区块.
tags:
  - MMU
---

> [GitHub: MEMBLOCK memblock_alloc_range()](https://github.com/BiscuitOS/HardStack/tree/master/Memory-Allocator/Memblock-allocator/API/memblock_alloc_range)
>
...
函数： memblock_alloc_range()

功能： 在指定范围内分配指定大小的物理内存区块
...
memblock_alloc_range() 函数用于从指定的内存区域内分配一块可用的物理内存。
...
+
+config DEBUG_MEMBLOCK_ALLOC_RANGE
+       bool "memblock_alloc_range()"
+
...
+#ifdef CONFIG_DEBUG_MEMBLOCK_ALLOC_RANGE
+	extern int bs_debug;
+	extern int debug_memblock_alloc_range(void);
+#endif
...
memblock_alloc_range() 函数分配一个大小为 0x100000 的内存区块，代码如下：
...
从运行的结果可以知道，memblock_alloc_range() 函数找到了一块起始地址为
