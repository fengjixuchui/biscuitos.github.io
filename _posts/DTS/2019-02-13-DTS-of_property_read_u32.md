---
layout: post
title:  "of_property_read_u32"
date:   2019-02-24 09:48:30 +0800
categories: [HW]
excerpt: DTS API of_property_read_u32().
tags:
  - DTS
---
...
> Github: [of_property_read_u32](https://github.com/BiscuitOS/HardStack/tree/master/Device-Tree/kernel/API/of_property_read_u32)
>
...
参数 np 指向当前设备节点；参数 propname 指向 u32 数据所在的属性名字；参数 
out_value 用于存储 u32 数据。

函数直接调用 of_property_read_u32_array() 函数读取 u32 数据。
...
形数组的属性，通过调用 of_property_read_u32() 函数获得整形数组属性值，最后打印
属性值，函数定义如下：
...
递。获得驱动对应的节点之后，通过调用 of_property_read_u32() 函数读取整数的属性
值，最后打印整数的值。驱动编写如下：
