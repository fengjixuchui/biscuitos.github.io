---
layout: post
title:  "memblock_add_range() 插入一块内存块"
date:   2019-03-11 18:17:30 +0800
categories: [MMU]
excerpt: memblock_add_range() 插入一块内存块.
tags:
  - MMU
---

> [GitHub: MEMBLOCK memblock_add_range()](https://github.com/BiscuitOS/HardStack/tree/master/Memory-Allocator/Memblock-allocator/API/memblock_add_range)
>
...
函数： memblock_add_range()

功能：将一块物理内存块插入到内存区里面，内存区可以是物理内存区，也可以是预留区。
具体调用树如下：
...
memblock_add_range() 函数的作用就是将一块物理内存区块加入到内存区内。本次实践的
目的是将新的内存区块加入到预留区内，该内存区块与已存在的内存区块完全重合，MEMBLOCK 是
否会将该内存区块合并到已存在的内存区块？
...
+
+config DEBUG_MEMBLOCK_ADD_RANGE
+       bool "memblock_add_range()"
+
...
+#ifdef CONFIG_DEBUG_MEMBLOCK_ADD_RANGE
+	extern int bs_debug;
+	extern int debug_memblock_add_range(void);
+#endif
...
Reserved-Region: 0x0 -- 0x0
Memory-Region:   0x60000000 - 0xa0000000
Scan region:
Reserved-Region: 0x60000000 - 0x60200000
...
往 memory 或者 reserved 区内插入一段内存区块是 memblock_add_range()
函数的基本功能，在驱动中，往预留区内添加一块预留内存区块，其地址范围是：
[0x60000000 - 0x60200000]，插入之后在将预留区的所有内存区块都打印出来，
其代码如下：
...
正如上面的代码，代码执行之后会往预留区内存添加一块内存区块，运行结果如下：
