...
> [Github: idr_remove](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/IDR/API/idr_remove)
>
...
idr_remove() 函数用于从 IDR 中移除一个 ID，并将与之绑定的指针接触绑定。参数 idr
指向 idr 根节点；参数 id 代表需要移除的 ID。函数调用 radix_tree_delete_item()
函数将 id 对应的节点从 radix-tree 中移除。
...
static __init int idr_demo_init(void)
{
	...
	idr_remove(&BiscuitOS_idr, idr_array[2]);
	printk("Remove ID: %d\n", idr_array[2]);
