...
> [Github: idr_find](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/IDR/API/idr_find)
>
...
idr_find() 函数用于通过 ID 查找与之绑定的指针。函数直接通过 radix_tree_lookup()
函数在 radix-tree 中查找指定的节点，如果找到则返回 ID 绑定的指针；反之返回 NULL。
...
static __init int idr_demo_init(void)
{
	...
	/* eturn pointer for given ID. */
	np = idr_find(&BiscuitOS_idr, idr_array[2]);
	printk("Found ID %d: %s\n", idr_array[2], np->name);
