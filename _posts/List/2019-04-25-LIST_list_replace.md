...
layout: post
title:  "list_replace"
date:   2019-04-26 08:47:30 +0800
categories: [HW]
excerpt: Bidirect-list list_replace().
tags:
  - Bidirect-list
---
...
> [Github: list_replace](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_replace)
>
...
list_replace() 函数通过传入新节点的指针和旧节点的指针，然后通过交换
两个节点的 prev 和 next 指针内容，以此达到节点替换的作用。
...
static __init int bindirect_demo_init(void)
{
	...
	list_replace(&node3.list, &node6.list);
...
在需要动态替换节点的地方，可以使用 list_replace() 函数进行替换。
