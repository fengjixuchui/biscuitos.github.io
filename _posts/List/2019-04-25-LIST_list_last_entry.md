---
layout: post
title:  "list_last_entry"
date:   2019-05-04 09:55:30 +0800
categories: [HW]
excerpt: Bidirect-list list_last_entry().
tags:
  - Bidirect-list
---
...
> [Github: list_last_entry](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_last_entry)
>
...
list_last_entry() 函数用于获得包含链表最后一个成员的结构体指针。参数 ptr 指向链表头;
type 参数指向用于包含链表的结构类型; member 参数用于指明链表成员在包含结构中的名字。
list_last_entry() 直接调用 list_entry() 函数，并将 ptr 链表的最后一个成员传递
到 list_entry() 函数中，以此获得包含其结构的指针。

###### list_entry

> [list_entry 源码分析](https://biscuitos.github.io/blog/LIST_list_entry/)
...
static __init int bindirect_demo_init(void)
{
	...
	np = list_last_entry(&BiscuitOS_list, struct node, list);
...
BiscuitOS_node5
BiscuitOS_node6
The entry struct node: BiscuitOS_node6
...
在实际需求中如果要获得最后一个链表成员对应的结构，那么可以考虑使用这个函数。
