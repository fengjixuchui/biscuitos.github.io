---
layout: post
title:  "list_for_each_entry_continue_reverse"
date:   2019-05-04 14:21:30 +0800
categories: [HW]
excerpt: Bidirect-list list_for_each_entry_continue_reverse().
tags:
  - Bidirect-list
---
...
> [Github: list_for_each_entry_continue_reverse](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_for_each_entry_continue_reverse)
>
...
参数 pos 指向特定的入口；head 指向双链表的表头；member 指明双链表在入口结构中的名字。
函数使用 for 循环，首先调用 list_prev_entry() 函数获得 pos 参数的前一个入口，继续
使用 list_prev_entry() 函数获得当前入口的前一个入口。
list_for_each_entry_continue_reverse() 通常配合 list_prepare_entry() 函数
使用。
...
static __init int bindirect_demo_init(void)
{
	...
	list_for_each_entry_continue_reverse(np2, &BiscuitOS_list, list)
		printk("%s\n", np2->name);
...
+       bool "list_for_each_entry_continue_reverse"
+
