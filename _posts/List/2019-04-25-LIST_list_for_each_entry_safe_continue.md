...
list_for_each_entry_safe_continue() 函数用于从指定位置安全遍历入口，所谓安全就是当
遍历过程中，涉及到删除当前节点的操作，如果不使用该函数，那么会引起内核 panic。参数 pos
指向指定入口；参数 n 指向下一个入口；head 指向双链表表头；member 用于指明双链表节点在
入口结构中的名字。函数调用一个 for 循环，首先调用两次 list_next_entry() 获得当前入口
和下一个入口；每次遍历调用 list_next_entry() 获得下一个入口。
...
static __init int bindirect_demo_init(void)
{
	...
	list_for_each_entry_safe_continue(np, n, &BiscuitOS_list, list) {
		printk("%s\n", np->name);
		list_del(&np->list);
	}
...
list_for_each_entry_safe_continue() 函数用于安全遍历移除节点操作。
