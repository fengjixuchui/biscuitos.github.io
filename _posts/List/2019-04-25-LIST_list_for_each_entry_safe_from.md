---
layout: post
title:  "list_for_each_entry_safe_from"
date:   2019-05-04 14:55:30 +0800
categories: [HW]
excerpt: Bidirect-list list_for_each_entry_safe_from().
tags:
  - Bidirect-list
---
...
list_for_each_entry_safe_from() 函数用于从指定入口开始，安全遍历剩余的入口。参数 pos
指向开始遍历的入口；n 参数指向下一个入口；head 指向链表表头；member 指明双链表节点在入口
结构中的名字。函数使用了一个 for 循环，首先调用 list_next_entry() 函数获得下一个入口，
在每次遍历过程中
...
+       bool "list_for_each_entry_safe_from"
+
