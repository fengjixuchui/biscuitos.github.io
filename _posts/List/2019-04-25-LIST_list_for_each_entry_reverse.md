---
layout: post
title:  "list_for_each_entry_reverse"
date:   2019-05-04 13:30:30 +0800
categories: [HW]
excerpt: Bidirect-list list_for_each_entry_reverse().
tags:
  - Bidirect-list
---
...
list_for_each_entry_reverse() 函数用于倒叙遍历内嵌双链表的结构体链表。参数 pos
指向当前遍历到的结构体指针；参数 head 指向双链表的表头；参数 member 指明双链表在结构中
的名字。函数使用 for 循环倒叙遍历双链表对应的结构体，首先使用 list_last_entry() 函数
获得双链表的最后一个节点，然后每次遍历使用 list_prev_entry() 函数获得前一个双链表
节点的入口。

###### list_last_entry

> [list_last_entry 源码分析](/blog/LIST_list_last_entry/)

###### list_prev_entry

> [list_prev_entry 源码分析](/blog/LIST_list_prev_entry/)
...
static __init int bindirect_demo_init(void)
{
	...
	list_for_each_entry_reverse(np, &BiscuitOS_list, list)
		printk("%s\n", np->name);
...
+       bool "list_for_each_entry_reverse"
+
...
list_for_each_entry_reverse() 函数用于倒叙遍历内嵌双链表的结构体。
