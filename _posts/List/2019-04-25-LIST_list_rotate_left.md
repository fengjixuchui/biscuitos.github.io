---
layout: post
title:  "list_rotate_left"
date:   2019-04-26 10:35:30 +0800
categories: [HW]
excerpt: Bidirect-list list_rotate_left().
tags:
  - Bidirect-list
---
...
> [Github: list_rotate_left](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_rotate_left)
>
...
list_rotate_left() 函数用于将链表第一节点移动到链表的末尾。函数首先调用
list_empty() 函数判断链表头是否为空。如果不空，则将链表中的第一个节点移动到
链表的末尾。
...
> [list_move_tail 源码](https://biscuitos.github.io/blog/LIST_list_move_tail/)
...
static __init int bindirect_demo_init(void)
{
	...
	list_rotate_left(&BiscuitOS_list);

	printk("Rotate BiscuitOS_list:\n");
	/* Traverser all node on bindirect-list */
	list_for_each_entry(np, &BiscuitOS_list, list)
		printk("%s\n", np->name);
...
在需要将链表第一个节点移动到链接末尾的情况，可以使用 list_rotate_left() 函数
实现这个目的。
