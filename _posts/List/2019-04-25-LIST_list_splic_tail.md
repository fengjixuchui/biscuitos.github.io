---
layout: post
title:  "list_splice_tail"
date:   2019-04-26 15:33:30 +0800
categories: [HW]
excerpt: Bidirect-list list_splice_tail().
tags:
  - Bidirect-list
---
...
> [Github: list_splice_tail](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_splice_tail)
>
...
list_splice_tail() 函数将 list 链表添加到 head 链表的末尾。参数 list 指向了一个
链表，参数 head 指向需要被插入的链表。函数首先通过 list_empty() 函数检查 list 链表
是否为空，如果空则直接返回；如果不空，则调用 __list_splice() 函数将 list 添加到
head 链表的表头。
...
> [__list_splice 源码](https://biscuitos.github.io/blog/LIST___list_splice/)
...
static __init int bindirect_demo_init(void)
{
	...
	/* join two list */
	list_splice_tail(&BiscuitOS_blist, &BiscuitOS_list);
...
list_splice_tail() 函数的作用可以实现队列的功能。
