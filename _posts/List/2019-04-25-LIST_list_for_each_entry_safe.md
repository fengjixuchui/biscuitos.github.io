---
layout: post
title:  "list_for_each_entry_safe"
date:   2019-05-04 14:55:30 +0800
categories: [HW]
excerpt: Bidirect-list list_for_each_entry_safe().
tags:
  - Bidirect-list
---
...
> [Github: list_for_each_entry_safe](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_for_each_entry_safe)
>
...
list_for_each_entry_safe() 函数用于安全正序遍历所有入口。在遍历过程中，如果需要
动态删除入口内嵌的双链表节点，可以使用这个函数安全删除。参数 pos 指向当前入口；参数 n
用于存储下一个入口；head 参数用于指向双链表的表头；member 参数用于指明双链表节点在
入口结构中的名字。函数使用 for 循环，首先调用 list_first_entry() 函数获得双链表
第一个入口，然后调用 list_next_entry() 函数获得下一个入口地址，在遍历中，调用
...
static __init int bindirect_demo_init(void)
{
	...
	list_for_each_entry_safe(np, n, &BiscuitOS_list, list) {
		printk("%s\n", np->name);
		list_del(&np->list);
	}
...
调用 list_for_each_entry_safe() 函数去删除节点并未引起内核 panic。
