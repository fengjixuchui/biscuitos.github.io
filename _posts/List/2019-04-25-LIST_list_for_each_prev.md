---
layout: post
title:  "list_for_each_prev"
date:   2019-05-04 11:10:30 +0800
categories: [HW]
excerpt: Bidirect-list list_for_each_prev().
tags:
  - Bidirect-list
---
...
> [Github: list_for_each_prev](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_for_each_prev)
>
...
list_for_each_prev() 函数用于倒叙遍历双链表的所有节点。参数 pos 指向当前遍历到的节点；
参数 head 指向链表的表头。函数调用 for 循环从链表的末尾倒叙遍历双链表中所有的节点。
...
static __init int bindirect_demo_init(void)
{
	...
	list_for_each_prev(np, &BiscuitOS_list)
...
当需要倒叙遍历双链表中所有节点时，可以使用 list_for_each_prev() 函数。
