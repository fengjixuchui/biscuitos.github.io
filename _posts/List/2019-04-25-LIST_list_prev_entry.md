---
layout: post
title:  "list_prev_entry"
date:   2019-05-04 10:40:30 +0800
categories: [HW]
excerpt: Bidirect-list list_prev_entry().
tags:
  - Bidirect-list
---
...
list_prev_entry() 函数用于获得成员的前一个成员。所有的成员都是由内嵌的双链表
构成，通过 list_prev_entry() 函数可以获得当前节点的前一个成员。pos 参数指向当前
成员指针；member 参数指向成员中链表的名字。函数直接调用 list_entry() 函数，将
成员的 pos->member.prev 传入，以此获得前一个双链表的成员，然后反推前一个成员。
...
list_prev_entry() 在内嵌双链表的结构中，可以便捷获得前一个成员。
