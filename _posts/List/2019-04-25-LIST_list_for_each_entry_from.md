---
layout: post
title:  "list_for_each_entry_from"
date:   2019-05-04 14:31:30 +0800
categories: [HW]
excerpt: Bidirect-list list_for_each_entry_from().
tags:
  - Bidirect-list
---
...
> [Github: list_for_each_entry_from](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_for_each_entry_from)
>
...
list_for_each_entry_from() 函数用于从指定入口开始，正序遍历所有的入口。每个入口都
内嵌一个双链表节点。pos 指向特定的入口；head 指向双链表表头；member 指明双链表节点
在入口节点的名字。函数使用 for 循环，从 pos 入口开始，调用 list_next_entry() 函数
获得下一个入口地址，正序遍历所有入口。

###### list_next_entry

> [list_next_entry 源码分析](/blog/LIST_list_next_entry/)
...
static __init int bindirect_demo_init(void)
{
	...
	list_for_each_entry_from(np, &BiscuitOS_list, list)
		printk("%s\n", np->name);
