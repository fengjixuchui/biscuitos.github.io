---
layout: post
title:  "list_is_singular"
date:   2019-04-26 10:46:30 +0800
categories: [HW]
excerpt: Bidirect-list list_is_singular().
tags:
  - Bidirect-list
---
...
> [Github: list_is_singular](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_is_singular)
>
...
list_is_singular() 函数判断链表是不是只含有一个节点。函数首先调用
list_empty() 函数判断当前链表不是空链表。然后判断 head 的 prev
和 next 都指向同一个位置。
...
static __init int bindirect_demo_init(void)
{
	...
	if (list_is_singular(&BiscuitOS_list))
		printk("list has just one entry.\n");
...
list_is_singular() 函数可以用于判断单一节点的链表。
