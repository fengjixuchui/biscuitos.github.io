...
layout: post
title:  "list_entry"
date:   2019-05-04 09:35:30 +0800
categories: [HW]
excerpt: Bidirect-list list_entry().
tags:
  - Bidirect-list
---
...
> [Github: list_entry](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_entry)
>
...
list_entry() 函数通过 struct list_head 对应的指针获得包含这个指针的结构体指针。
ptr 参数指向 struct list_head 指针； type 参数表示包含 list_head 指针的结构类型；
member 参数用于指明 list_head 指针在 type 结构中的名字。通过这个函数可以有 list_head
指针反推出包含该指针的结构。

###### container_of

> [container_of 源码分析](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_splice_tail_init)
...
利用 list_entry() 函数可以便捷获得包含其的结构体指针。
