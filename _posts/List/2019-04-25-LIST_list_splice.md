...
layout: post
title:  "list_splice"
date:   2019-04-26 15:05:30 +0800
categories: [HW]
excerpt: Bidirect-list list_splice().
tags:
  - Bidirect-list
---
...
> [Github: list_splice](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_splice)
>
...
list_splice() 函数用于将链表 list 添加到 head 链表的头部。函数首先检查 list
链表是否为空，如果为空，则返回；反之调用 __list_splice() 将 list 添加到
head 的头部。
...
static __init int bindirect_demo_init(void)
{
	...
	/* join two list */
	list_splice(&BiscuitOS_blist, &BiscuitOS_list);
...
从实践结果可以看出，list_splice() 函数将新链表添加到原始链表的头部，这适合于
向下生长的堆栈，所以在堆栈有用到。
