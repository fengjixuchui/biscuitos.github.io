---
layout: post
title:  "__list_del_entry_valid"
date:   2019-04-26 08:15:30 +0800
categories: [HW]
excerpt: Bidirect-list __list_del_entry_valid().
tags:
  - Bidirect-list
---
...
__list_del_entry_valid() 函数的作用是在即将要删除一个节点之前，检查这个节点是否
可用。该函数的实现与 CONFIG_DEBUG_LIST 宏有关，如果这个宏定义，那么
__list_del_entry_valid() 函数的实现如下：
...
static __init int bindirect_demo_init(void)
{
	...
	/* Delete a entry */
	if (__list_del_entry_valid(&node6.list)) {
...
在删除一个节点之前，应该调用 __list_del_entry_valid() 函数检查节点的有效性，然后
调用 __list_del() 函数进行删除。
