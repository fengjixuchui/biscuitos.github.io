...
layout: post
title:  "__list_splice"
date:   2019-04-26 14:57:30 +0800
categories: [HW]
excerpt: Bidirect-list __list_splice().
tags:
  - Bidirect-list
---
...
__list_splice() 函数用于将节点插入到链表里。函数将 list 对应的链表插入到
prev 节点之后。函数首先获得 list 节点的 next 和 prev 节点，然后将节点 list
插入到 prev 节点之后，调整 next 节点。
...
static __init int bindirect_demo_init(void)
{
	...
	/* join two list */
	__list_splice(&BiscuitOS_blist, &BiscuitOS_list,
				BiscuitOS_list.next);
