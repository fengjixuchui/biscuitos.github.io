...
date:   2019-05-04 09:45:30 +0800
categories: [HW]
excerpt: Bidirect-list list_first_entry().
tags:
  - Bidirect-list
---
...
> [Github: list_first_entry](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_first_entry)
>
...
list_first_entry() 函数用于获得包含链表第一个成员的结构体指针。ptr 参数指向链表头;
type 参数指明包含链表成员的结构类型;member 参数指明了链表成员在包含结构体中的名字。
list_first_entry() 函数直接调用 list_entry() 函数，将 ptr->next, 即链表的第一个
成员指针传递到 list_entry() 函数中，以此获得包含链表结构的指针。
...
list_first_entry() 函数可以快速获得链表第一个成员对应的包含结构。
