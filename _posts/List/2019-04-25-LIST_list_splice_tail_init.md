---
layout: post
title:  "list_splice_tail_init"
date:   2019-05-04 09:14:30 +0800
categories: [HW]
excerpt: Bidirect-list list_splice_tail_init().
tags:
  - Bidirect-list
---
...
> [Github: list_splice_tail_init](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/list/bindirect-list/API/list_splice_tail_init)
>
...
list_splice_tail_init() 函数用于将 list 链表成员合并到 head 链表末尾，并初始化
list 链表。参数 list 指向需要被合并的链表；head 参数指向链表的表头。函数首先通过
list_empty() 函数检查 list 链表是不是空的，如果 list 空则直接返回；如果 list 链表
不空，则调用 __list_splice() 函数将 list 链表的成员合并到 head 链表的末尾，最后
调用 INIT_LIST_HEAD() 函数初始化 list 链表。

###### list_empty

> [list_empty 源码](https://biscuitos.github.io/blog/LIST_list_empty/)

###### __list_splice

> [\_\_list_splice 源码](https://biscuitos.github.io/blog/LIST___list_splice/)
...
static __init int bindirect_demo_init(void)
{
	...
	list_splice_tail_init(&BiscuitOS_blist, &BiscuitOS_list);
	if (list_empty(&BiscuitOS_blist))
		printk("BiscuitOS_blist is empty list.\n");
...
从实践结果看出，list_splice_tail_init() 函数不仅将 list 的成员合并到 head 链表里，
并且初始化了 list 链表。
