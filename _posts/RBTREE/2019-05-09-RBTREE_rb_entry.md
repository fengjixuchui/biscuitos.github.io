...
> [Github: rb_entry](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/rbtree/API/rb_entry)
>
...
rb_entry() 函数用于获得包含 rb_node 的数据结构。参数 ptr 指向 rb_node 的指针；
参数 type 用于说明包含 rb_node 的结构类型；参数 member 用于说明 rb_node 在包含
 rb_node 结构中的名字。函数实际通过调用 container_of() 函数实现。
...
很有用的接口去获得包含 struct rb_node 的结构。
