---
layout: post
title:  "memblock_phys_alloc_nid() 从指定 NUMA 节点中分配指定物理内存"
date:   2019-03-13 14:45:30 +0800
categories: [MMU]
excerpt: memblock_phys_alloc_nid() 从指定 NUMA 节点中分配指定物理内存.
tags:
  - MMU
---

> [GitHub: MEMBLOCK memblock_phys_alloc_nid()](https://github.com/BiscuitOS/HardStack/tree/master/Memory-Allocator/Memblock-allocator/API/memblock_phys_alloc_nid)
>
...
memblock_phys_alloc_nid() 函数用于从指定的 NUMA 节点中分配物理内存，
...
#ifdef CONFIG_DEBUG_MEMBLOCK_PHYS_ALLOC_NID
int __init debug_memblock_phys_alloc_nid(void)
{
	 ...
	 */
	addr = memblock_phys_alloc_nid(0x100000, 0x1000, NUMA_NO_NODE);
...
+
+config DEBUG_MEMBLOCK_PHYS_ALLOC_NID
+       bool "memblock_phys_alloc_nid()"
+
...
+#ifdef CONFIG_DEBUG_MEMBLOCK_PHYS_ALLOC_NID
+	extern int bs_debug;
+	extern int debug_memblock_phys_alloc_nid(void);
+#endif
...
memblock_phys_alloc_nid() 函数分配一个大小为 0x100000 的内存区块，
代码如下：
...
从运行的结果可以知道，memblock_phys_alloc_nid() 函数找到了一块起始地址为
