---
layout: post
title:  "linux 4.8.1 arm64 Usermanual"
date:   2019-02-20 08:51:30 +0800
categories: [Build]
excerpt: linux 4.8.1 arm64 Usermanual.
tags:
  - Linux
---
...
>
> - [配置 linux 4.8.1 arm64 内核](#配置 linux 4.8.1 arm64 内核)
>
> - [编译 linux 4.8.1 arm64 内核](#编译 linux 4.8.1 arm64 内核)
>
> - [运行 linux 4.8.1 arm64 内核](#运行 linux 4.8.1 arm64 内核)
>
> - [驱动开发](#驱动开发)
>
> - [调试 linux 4.8.1 arm64 内核](#linux 4.8.1 arm64 内核调试)
>
> - [附录](#附录)
...
获得 BiscuitOS 项目之后，可以使用 BiscuitOS 构建 linux 4.8.1 arm64 的开发环境。开发者
只需执行如下命令就可以获得 linux 4.8.1 arm64 完整的 BiscuitOS，如下：

{% highlight bash %}
cd BiscuitOS
make linux-4.8.1-arm64_defconfig
make
{% endhighlight %}
...
***********************************************
Output:
 BiscuitOS/output/linux-4.8.1-aarch 

linux:
 BiscuitOS/output/linux-4.8.1-aarch/linux/linux 

README:
 BiscuitOS/output/linux-4.8.1-aarch/README.md 
...
```
cd BiscuitOS/output/linux-4.8.1-aarch/linux/linux
...
make ARCH=arm64 CROSS_COMPILE=BiscuitOS/output/linux-4.8.1-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- Image -j8
```

# Build Busybox

```
cd BiscuitOS/output/linux-4.8.1-aarch/busybox/busybox
make clean
make menuconfig
  Busybox Settings --->
    Build Options --->
...
make CROSS_COMPILE=BiscuitOS/output/linux-4.8.1-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- -j8

make CROSS_COMPILE=BiscuitOS/output/linux-4.8.1-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu- install
```
...
```
cd BiscuitOS/output/linux-4.8.1-aarch
...
```
BiscuitOS/output/linux-4.8.1-aarch/aarch64-linux-gnu/aarch64-linux-gnu/bin/aarch64-linux-gnu-gdb BiscuitOS/output/linux-4.8.1-aarch/linux/linux/vmlinux
...
#### <span id="配置 linux 4.8.1 arm64 内核">配置 linux 4.8.1 arm64 内核</span>
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000171.png)

选择 **General setup**

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000172.png)

选中 **Initial RAM filesystem and RAM disk (initramfs/initrd) support**

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000173.png)

选择 **Device Driver**

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000174.png)

选择 **Block devices**

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000175.png)

选中 **RAM block device support**, 并设置 **Default RAM disk size** 的大小为 
153600 . 保存并退出，至此，Linux 配置完成。

#### <span id="编译 linux 4.8.1 arm64 内核">编译 linux 4.8.1 arm64 内核</span>
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000176.png)
...
#### <span id="运行 linux 4.8.1 arm64 内核">运行 linux 4.8.1 arm64 内核</span>

完成上面的步骤之后，开发者就可以运行 linux 4.8.1 arm64 内核，使用如下命令即可：
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000177.png)

至此，一个 linux 4.8.1 arm64 内核已经运行，开发者可以根据自己兴趣和需求对内核进行魔改。
...
持 linux 4.8.1 arm64 内核驱动的开发。开发者可以参照下面方法进行更多驱动开发。
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000178.png)

首先在目录中找到 **Device Driver --->** 回车并进入其中。

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000179.png)

接着在目录中找到 **BiscuitOS Driver --->** 按 Y 选中并按回车键进入。

![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000180.png)
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000181.png)
...
# <span id="linux 4.8.1 arm64 内核调试">linux 4.8.1 arm64 内核调试</span>
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000182.png)
...
![LINUXP](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/kernel/BUDX000183.png)
