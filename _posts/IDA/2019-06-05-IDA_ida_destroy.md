...
> [Github: ida_destroy](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/IDA/API/ida_destroy)
>
...
ida_destroy() 函数用于释放所有的 ID。参数 ida 指向 IDA。函数调用 xa_lock_irqsave()
函数加锁，然后调用 radix_tree_for_each_slot() 遍历 IDA radix-tree 树中的
所有 slot，然后获得 slot 指向的 bitmap，如果 bitmap 不是 exceptional 节点，
那么直接调用 kfree 进行释放。接着调用 radix_tree_iter_delete() 将对应的 slot
从 radix-tree 中抹除。遍历完毕之后，调用 xa_unlock_irqrestore() 解锁。
