...
> [Github: bitmap_zero](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/bitmap/API/bitmap_zero)
>
...
bitmap_zero() 函数用于清除 bitmap 中指定 bits。参数 dst 指向 bitmap；参数
nbits 指向 bits 数。函数首先使用 BITS_TO_LONGS() 函数获得 bitmap 占用 long
的个数，然后将该值乘与 unsigned long 的长度。然后调用 memset 直接将 bitmap
按 long 长度进行清零操作。
