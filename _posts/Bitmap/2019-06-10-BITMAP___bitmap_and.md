...
__bitmap_and() 函数用于两个 bitmap 之间的与运算。参数 dst 用于存储相与的结果；
参数 bitmap1 指向第一个 bitmap；参数 bitmap2 指向第二个 bitmap；参数 bits
指定从低位开始，需要相与的位数。函数首先计算 bits 占用了 BITS_PER_LONG 的个数，
接着调用第一个 for 循环，循环一共直到遍历完所有的整数个 long，每次遍历，都讲
两个 bitmap 相与的值存储在 dst。循环结束之后，如果 bits 包含多余的 bit，那么
就将两个 bitmap 相与之后，在与 BITMAP_LAST_WORD_MASK() 相与以此得到剩余的几个
bit。
