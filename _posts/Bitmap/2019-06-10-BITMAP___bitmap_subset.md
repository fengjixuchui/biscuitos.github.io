...
__bitmap_subset() 函数用于比较子集，如果 bitmap1 在指定的 bits 位数是 bitmap2
指定位数含 1 的子集，那么返回 1；反之返回 0. 参数 bitmap1 指向第一个 bitmap；参数
bitmap2 指向第二个 bitmap。参数 bits 代表检查子集的范围。函数首先将 bits 除以
BITS_PER_LONG，以此计算出 bits 占用了多少个完整的 long 变量，然后在这些完整的 long
变量里进行与非操作，如果与非的结果为 1，那么代表没有子集；反之继续比较，如果 bits
函数不足 long 的 bits，那么将不足 bits 所在 long 变量进行与非操作，将操作的结果
与 BITMAP_LAST_WORD_MASK() 进行相与，如果结果为 1，代表没有 1 对应的子集，则返回
0；反之返回 1，代表 bitmap1 在 bitmap2 指定的位数中，函数 1 的子集。
