---
layout: post
title:  "test_and_change_bit"
date:   2019-06-10 05:30:30 +0800
categories: [HW]
excerpt: BITMAP test_and_change_bit().
tags:
  - Tree
---
...
> [Github: test_and_change_bit](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/bitmap/API/test_and_change_bit)
>
...
test_and_change_bit() 函数的作用是将指定位翻转。参数 nr 在 BITS_PER_LONG
范围内，用于指定翻转的位置；参数 p 指向 bit。test_and_change_bit() 属于
bitops 类中的一个，其实现与体系有关，上方的函数是通用定义，下方的函数是与体系
有关的定义。对于通用的定义，函数首先计算 nr 位于 bit 的 long 偏移，然后调用
BIT_MASK() 生成对于的掩码，最后使用 atomic_long_fetch_xor() 函数写原子
或操作，设置对应的 bit。对于体系相关的实现，对 ARM 而言，其实现与下面函数有关：
