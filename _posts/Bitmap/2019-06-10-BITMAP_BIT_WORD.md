...
BIT_WORD 宏用于计算 nr 个 bit 占用多少个字，这里字的长度是 BITS_PER_LONG，且
计算出来的字长度比实际少一个，这也是一种向下对齐。参数 nr 代表 bit 的总数。函数
将 nr 的值除以 BITS_PER_LONG 以此向下计算出 nr 个 bit 占了多少个字。
