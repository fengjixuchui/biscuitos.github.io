...
bitmap_fill() 函数的作用是将 dst 对应的 bitmap 的 nbits 全部置位。参数 dst
指向 bitmap；参数 nbits 指向需要置位的位数。函数首先通过 BITS_TO_LONGS() 函数
计算出包含 nbits 需要的最小 unsignd long 个数。然后将需要的字节数通过 memset
函数设置为 0xff。
...
0x90911010 fill bitmap: 0xffffffff
