---
layout: post
title:  "read_cpuid_revision"
date:   2019-05-09 14:55:30 +0800
categories: [HW]
excerpt: CPUID read_cpuid_revision().
tags:
  - CPUID
---
...
read_cpuid_revision() 函数的作用就是读取 CPU revision 信息。其操作的的
寄存器 MIDR 寄存器，更多信息请参考
...
static __init int cpuid_demo_init(void)
{
	...
	info = read_cpuid_revision();

	printk("CPU revision: %#x\n", info);
...
cpuid_demo_init() 读取 revision 信息。
