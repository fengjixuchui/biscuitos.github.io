...
> [Github: atomic_andnot](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/atomic/API/atomic_andnot)
>
...
atomic_andnot() 用于给 atomic_t 变量清除特定位。在 ARMv7 中，使用 ATOMIC_OPS 宏定义
了 atomic_andnot() 函数。开发者可以通过编译之后的结果查看 atomic_andnot() 函数的实现，
如下：
...
atomic_andnot() 函数的定义如上，参数 i 指明 atomic 变量需要清除的位；参数 v 指向
atomic_t 变量。函数首先使用 prefetchw() 函数将 v->counter 的值预读到 cache，
然后调用一个内嵌汇编，汇编首先调用 ldrex 指令首先对 v->counter 对应的内存地址
设置独占标志，同时从内存中读取 v->counter 的值到 result。接着调用 andnot 指令，
...
/*
 * atomic_andnot (ARMv7 Cotex-A9MP)
 *
 * static inline void atomic_andnot(int i, atomic_t *v)
 * {
 *         unsigned long tmp;
...
{
	/* Atomic andnot */
	atomic_andnot(1, &BiscuitOS_counter);
...
当需要对一个 atomic_t 变量做位清除操作时候，可以使用 atomic_andnot() 函数。
