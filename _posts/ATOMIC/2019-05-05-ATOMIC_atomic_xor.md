...
layout: post
title:  "atomic_xor"
date:   2019-05-06 08:37:30 +0800
categories: [HW]
excerpt: ATOMIC atomic_xor().
tags:
  - ATOMIC
---
...
> [Github: atomic_xor](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/atomic/API/atomic_xor)
>
...
atomic_xor() 用于给 atomic_t 变量做异或运算。在 ARMv7 中，使用 ATOMIC_OPS 宏定义
了 atomic_xor() 函数。开发者可以通过编译之后的结果查看 atomic_xor() 函数的实现，
如下：
...
当需要对一个 atomic_t 变量做异或运算的时候，可以使用 atomic_xor() 函数。
