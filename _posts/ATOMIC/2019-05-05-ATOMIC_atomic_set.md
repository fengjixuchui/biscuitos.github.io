...
> [Github: atomic_set](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/atomic/API/atomic_set)
>
...
atomic_set() 函数用于设置一个 atomic 变量的值。参数 v 指向一个 atomic 指针；
i 用于存储要设置的值。函数直接调用 WRITE_ONCE() 将 atomic 变量的值写入到内存中。

###### WRITE_ONCE

> [WRITE_ONCE 源码分析](https://biscuitos.github.io/blog/ATOMIC_WRITE_ONCE/)
...
{
	atomic_set(&BiscuitOS_counter, 6);
...
动态修改一个 atomic 的值就使用 atomic_set() 函数。
