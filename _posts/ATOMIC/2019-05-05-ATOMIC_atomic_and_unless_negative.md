...
atomic_inc_unless_negative() 函数用于对非负数的 atomic_t 变量执行加一操作。
如果 atomic_t 变量的值为负数，atomic_inc_unless_negative() 对其不起作用。
函数首先判断 atomic_t 变量的值是否为负数，如果是直接返回。如果不是则调用
atomic_try_cmpxchg() 函数加一操作。
