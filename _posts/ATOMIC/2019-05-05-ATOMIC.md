...
> [Linux 5.0 arm32 开发环境搭建](/blog/Linux-5.0-arm32-Usermanual/)
...
> [atomic_add: atomic 无条件加法函数](/blog/ATOMIC_atomic_add/)
>
> [atomic_add_negative: atomic 加法操作并判断结果是否为负数](/blog/ATOMIC_atomic_and_negative/)
>
> [atomic_add_return: atomic 加法操作并返回结果](/blog/ATOMIC_atomic_add_return/)
>
> [atomic_add_return_relaxed: atomic 加法操作并返回结果](/blog/ATOMIC_atomic_add_return_relaxed/)
>
> [atomic_add_unless: atomic 不同值加法](/blog/ATOMIC_atomic_add_unless/)
>
> [atomic_add_unless_negative: atomic 非负数加法](/blog/ATOMIC_atomic_add_unless_negative/)
>
> [atomic_fetch_add: 读取 atomic 原始值再进行加法操作](/blog/ATOMIC_atomic_fetch_add/)
>
> [atomic_fetch_add_relaxed: 读取 atomic 原始值再进行加法操作](/blog/ATOMIC_atomic_fetch_add_relaxed/)
>
> [atomic_fetch_add_unless: 读取 atomic 原始值再进行不同值加法](/blog/ATOMIC_atomic_fetch_add_unless/)

### <span id="atomic 原子操作减法操作">atomic 原子操作减法操作</span>

Linux 为 atomic 原子操作提供了无条件加法运算和有条件减法运行，分别如下：

> [atomic_sub: atomic 无条件减法](/blog/ATOMIC_atomic_sub/)
>
> [atomic_sub_and_test: atomic 减法操作并判断结果是否为 0](/blog/ATOMIC_atomic_sub_and_test/)
>
> [atomic_sub_return: atomic 减法操作并返回结果](/blog/ATOMIC_atomic_sub_return/)
>
> [atomic_sub_return_relaxed: atomic 减法操作并返回结果](/blog/ATOMIC_atomic_sub_return_relaxed/)
>
> [atomic_fetch_sub: 先读取 atomic 的值再进行减法](/blog/ATOMIC_atomic_fetch_sub/)
>
> [atomic_fetch_sub_relaxed: 先读取 atomic 的值再进行减法](/blog/ATOMIC_atomic_fetch_sub_relaxed/)

### <span id="atomic 原子操作加一操作">atomic 原子操作加一操作</span>

Linux 为 atomic 原子操作提供了无条件加法运算和有条件加一运行，分别如下：

> [atomic_inc: atomic 加一操作](/blog/ATOMIC_atomic_inc/)
>
> [atomic_inc_and_test: atomic 加一操作并判断结果是否为 0](/blog/ATOMIC_atomic_inc_and_test/)
>
> [atomic_inc_not_zero: atomic 非零的加一操作](/blog/ATOMIC_atomic_inc_not_zero/)
>
> [atomic_inc_return: atomic 加一操作并返回结果](/blog/ATOMIC_atomic_inc_return/)
>
> [atomic_fetch_inc: 先读取 atomic 的值，然后进行加一操作](/blog/ATOMIC_atomic_fetch_inc/)

### <span id="atomic 原子操作减一操作">atomic 原子操作减一操作</span>

Linux 为 atomic 原子操作提供了无条件加法运算和有条件减一运行，分别如下：

> [atomic_dec: atomic 减一操作](/blog/ATOMIC_atomic_dec/)
>
> [atomic_dec_and_test: atomic 减一操作并判断结果是否为 0](/blog/ATOMIC_atomic_dec_and_test/)
>
> [atomic_dec_if_positive: atomic 正数减一操作](/blog/ATOMIC_atomic_dec_if_positive/)
>
> [atomic_dec_return: atomic 减一操作并返回结果](/blog/ATOMIC_atomic_dec_return/)
>
> [atomic_dec_unless_positive: atomic 的非正数减一操作](/blog/ATOMIC_atomic_dec_unless_positive/)
>
> [atomic_fetch_dec: 先读取 atomic 值再进行减一操作](/blog/ATOMIC_atomic_fetch_dec/)

### <span id="atomic 原子操作 AND 运算">atomic 原子操作 AND 运算</span>

Linux 为 atomic 原子操作提供了按位 AND 操作，分别如下：

> [atomic_and: atomic 按位与操作](/blog/ATOMIC_atomic_and/)
>
> [atomic_fetch_and: 先读取 atomic 的值在按位与操作](/blog/ATOMIC_atomic_fetch_and/)
>
> [atomic_fetch_and_relaxed: 先读取 atomic 的值在按位与操作](/blog/ATOMIC_atomic_fetch_and_relaxed/)

### <span id="atomic 原子操作 OR 运算">atomic 原子操作 OR 运算</span>

Linux 为 atomic 原子操作提供了按位 OR 操作，分别如下：

> [atomic_or: atomic 或运算](/blog/ATOMIC_atomic_or/)
>
> [atomic_fetch_or: 先读取 atomic 的值再进行或运算](/blog/ATOMIC_atomic_fetch_or/)
>
> [atomic_fetch_or_relaxed: 先读取 atomic 的值再进行或运算](/blog/ATOMIC_atomic_fetch_or_relaxed/)

### <span id="atomic 原子操作 XOR 运算">atomic 原子操作 XOR 运算</span>

Linux 为 atomic 原子操作提供了按位 XOR 操作，分别如下：

> [atomic_xor: atomic 异或运算](/blog/ATOMIC_atomic_xor/)
>
> [atomic_fetch_xor: 先读取 atomic 的值再进行异或运算](/blog/ATOMIC_atomic_fetch_xor/)
>
> [atomic_fetch_xor_relaxed: 先读取 atomic 的值再进行异或运算](/blog/ATOMIC_atomic_fetch_xor_relaxed/)

### <span id="atomic 原子操作按位清除运算">atomic 原子操作按位清除运算</span>

Linux 为 atomic 原子操作提供了清除特定位操作，分别如下：

> [atomic_andnot: atomic 清除特定位](/blog/ATOMIC_atomic_andnot/)
>
> [atomic_fetch_andnot: 先读取 atomic 的值再清除特定位](/blog/ATOMIC_atomic_fetch_andnot/)
>
> [atomic_fetch_andnot_relaxed: 先读取 atomic 的值再清除特定位](/blog/ATOMIC_atomic_fetch_andnot_relaxed/)

### <span id="atomic 原子操作读取/设置值">atomic 原子操作读取/设置值</span>

Linux 为 atomic 原子操作提供了直接读值和设置值的操作，分别如下：

> [atomic_read: atomic 读值操作](/blog/ATOMIC_atomic_read/)
>
> [atomic_set: atomic 设置值操作](/blog/ATOMIC_atomic_set/)

### <span id="atomic 原子操作交换运算">atomic 原子操作交换运算</span>

Linux 为 atomic 原子操作提供了值交换操作，分别如下：

> [atomic_cmpxchg: atomic 比较交换运算](/blog/ATOMIC_atomic_cmpxchg/)
>
> [atomic_xchg: atomic 交换运算](/blog/ATOMIC_atomic_xchg/)
>
> [atomic_cmpxchg_relaxed: atomic 比较交换运算](/blog/ATOMIC_atomic_cmpxchg_relaxed/)

### <span id="atomic 原子操作相关操作">atomic 原子操作相关操作</span>

> [READ_ONCE: 从内存中读取数据](/blog/ATOMIC_READ_ONCE/)
>
> [WRITE_ONCE: 向内存中写值](/blog/ATOMIC_WRITE_ONCE/)
>
> [prefetchw: 从内存预读数据到 cache](/blog/ATOMIC_prefetchw/)
>
> [\_\_read_once_size: 按 size 从内存读值](/blog/ATOMIC___read_once_size/)
>
> [\_\_read_once_size_nocheck: 按 size 从内存读值不检查](/blog/ATOMIC___read_once_size_nocheck/)
>
> [\_\_write_once_size: 按 size 向内存写值](/blog/ATOMIC___write_once_size/)
...
> [atomic_add](/blog/ATOMIC_atomic_add/)
>
> [atomic_add_negative](/blog/ATOMIC_atomic_add_negative/)
>
> [atomic_add_return](/blog/ATOMIC_atomic_add_return/)
>
> [atomic_add_return_relaxed](/blog/ATOMIC_atomic_add_return_relaxed/)
>
> [atomic_add_unless](/blog/ATOMIC_atomic_add_unless/)
>
> [atomic_add_unless_negative](/blog/ATOMIC_atomic_add_unless_negative/)
>
> [atomic_and](/blog/ATOMIC_atomic_and/)
>
> [atomic_andnot](/blog/ATOMIC_atomic_andnot/)
>
> [atomic_cmpxchg](/blog/ATOMIC_atomic_cmpxchg/)
>
> [atomic_cmpxchg_relaxed](/blog/ATOMIC_atomic_cmpxchg_relaxed/)
>
> [atomic_dec](/blog/ATOMIC_atomic_dec/)
>
> [atomic_dec_and_test](/blog/ATOMIC_atomic_dec_and_test/)
>
> [atomic_dec_if_positive](/blog/ATOMIC_atomic_dec_if_positive/)
>
> [atomic_dec_return](/blog/ATOMIC_atomic_dec_return/)
>
> [atomic_dec_unless_positive](/blog/ATOMIC_atomic_dec_unless_positive/)
>
> [atomic_fetch_add](/blog/ATOMIC_atomic_fetch_add/)
>
> [atomic_fetch_add_relaxed](/blog/ATOMIC_atomic_fetch_add_relaxed/)
>
> [atomic_fetch_add_unless](/blog/ATOMIC_atomic_fetch_add_unless/)
>
> [atomic_fetch_and](/blog/ATOMIC_atomic_fetch_and/)
>
> [atomic_fetch_andnot](/blog/ATOMIC_atomic_fetch_andnot/)
>
> [atomic_fetch_andnot_relaxed](/blog/ATOMIC_atomic_fetch_andnot_relaxed/)
>
> [atomic_fetch_and_relaxed](/blog/ATOMIC_atomic_fetch_and_relaxed/)
>
> [atomic_fetch_dec](/blog/ATOMIC_atomic_fetch_dec/)
>
> [atomic_fetch_inc](/blog/ATOMIC_atomic_fetch_inc/)
>
> [atomic_fetch_or](/blog/ATOMIC_atomic_fetch_or/)
>
> [atomic_fetch_or_relaxed](/blog/ATOMIC_atomic_fetch_or_relaxed/)
>
> [atomic_fetch_sub](/blog/ATOMIC_atomic_fetch_sub/)
>
> [atomic_fetch_sub_relaxed](/blog/ATOMIC_atomic_fetch_sub_relaxed/)
>
> [atomic_fetch_xor](/blog/ATOMIC_atomic_fetch_xor/)
>
> [atomic_fetch_xor_relaxed](/blog/ATOMIC_atomic_fetch_xor_relaxed/)
>
> [atomic_inc](/blog/ATOMIC_atomic_inc/)
>
> [atomic_inc_and_test](/blog/ATOMIC_atomic_inc_and_test/)
>
> [atomic_inc_not_zero](/blog/ATOMIC_atomic_inc_not_zero/)
>
> [atomic_inc_return](/blog/ATOMIC_atomic_inc_return/)
>
> [ATOMIC_INIT](/blog/ATOMIC_ATOMIC_INIT/)
>
> [atomic_or](/blog/ATOMIC_atomic_or/)
>
> [atomic_read](/blog/ATOMIC_atomic_read/)
>
> [atomic_set](/blog/ATOMIC_atomic_set/)
>
> [atomic_sub](/blog/ATOMIC_atomic_sub/)
>
> [atomic_sub_and_test](/blog/ATOMIC_atomic_sub_and_test/)
>
> [atomic_sub_return](/blog/ATOMIC_atomic_sub_return/)
>
> [atomic_sub_return_relaxed](/blog/ATOMIC_atomic_sub_return_relaxed/)
>
> [atomic_xchg](/blog/ATOMIC_atomic_xchg/)
>
> [atomic_xor](/blog/ATOMIC_atomic_xor/)
>
> [prefetchw](/blog/ATOMIC_prefetchw/)
>
> [\_\_READ_ONCE](/blog/ATOMIC___READ_ONCE/)
>
> [READ_ONCE](/blog/ATOMIC_READ_ONCE/)
>
> [\_\_read_once_size](/blog/ATOMIC___read_once_size/)
>
> [\_\_read_once_size_nocheck](/blog/ATOMIC___read_once_size_nocheck/)
>
> [WRITE_ONCE](/blog/ATOMIC_WRITE_ONCE/)
>
> [\_\_write_once_size](/blog/ATOMIC___write_once_size/)
