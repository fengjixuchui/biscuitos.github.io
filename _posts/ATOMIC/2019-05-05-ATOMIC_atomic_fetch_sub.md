...
date:   2019-05-07 07:50:30 +0800
categories: [HW]
excerpt: ATOMIC atomic_fetch_sub().
tags:
  - ATOMIC
---
...
> [Github: atomic_fetch_sub](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/atomic/API/atomic_fetch_sub)
>
...
atomic_fetch_sub() 用于获得 atomic_t 变量的原始值，并给 atomic_t 变量做
减法。在 ARMv7 中，使用 ATOMIC_OPS 宏定义了 atomic_fetch_sub() 函数。开
发者可以通过编译之后的结果查看 atomic_fetch_sub() 函数的实现，如下：

{% highlight ruby %}
static inline void atomic_fetch_sub(int i, atomic_t *v)
{
        unsigned long tmp;
        int result, val;
...
atomic_fetch_sub() 函数的定义如上，参数 i 指明 atomic 变量需要减少的值；
...
static __init int atomic_demo_init(void)
{
	...
	/* Atomic sub */
	val = atomic_fetch_sub(1, &BiscuitOS_counter);
...
atomic_fetch_sub() 函数可以用于获得减法之前 atomic_t 变量的值。
