---
layout: post
title:  "atomic_fetch_andnot_relaxed"
date:   2019-05-07 07:59:30 +0800
categories: [HW]
excerpt: ATOMIC atomic_fetch_andnot_relaxed().
tags:
  - ATOMIC
---
...
> [Github: atomic_fetch_andnot_relaxed](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/atomic/API/atomic_fetch_andnot_relaxed)
>
...
atomic_fetch_andnot_relaxed() 用于获得 atomic_t 变量的原始值，并清除 atomic_t 变量的指定位。
在 ARMv7 中，使用 ATOMIC_OPS 宏定义了 atomic_fetch_andnot_relaxed() 函数。开
发者可以通过编译之后的结果查看 atomic_fetch_andnot_relaxed() 函数的实现，如下：

{% highlight ruby %}
static inline void atomic_fetch_andnot_relaxed(int i, atomic_t *v)
{
        unsigned long tmp;
        int result, val;
...
atomic_fetch_andnot_relaxed() 函数的定义如上，参数 i 指明 atomic 变量需要清除的位；
...
static __init int atomic_demo_init(void)
{
	...
	/* Atomic andnot */
	val = atomic_fetch_andnot_relaxed(1, &BiscuitOS_counter);
...
+       bool "atomic_fetch_andnot_relaxed"
+
...
atomic_fetch_andnot_relaxed() 函数可以用于获得清除特定位之前 atomic_t 变量的值。
