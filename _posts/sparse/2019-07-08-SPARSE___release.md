...
> [Github: __release](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/sparse/API/__release)
>
...
__release() 宏与 __acquire() 成对使用，用于检查引用计数减一操作是否
和减一操作成对存在，如果只有减一操作，那么 sparse 工具就会报错。
...
在编译之前，如果 __release() 与 __release() 不成对使用，那么 sparse
工具就会报错。
