...
> [Github: __kernel](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/sparse/API/__kernel)
>
...
__kernel 用于限定指针只能指向内核空间。如果指针指向其他空间，比如用户空间，
或者 I/O 空间，那么 sparse 就是警告。__attribute__((address_space(0)))，
是用来修饰一个变量的，而且变量所在的地址空间必须是 0，即内核空间。这里把程
...
drivers/BiscuitOS/sparse.c:22:13:    expected int *src
drivers/BiscuitOS/sparse.c:22:13:    got int [noderef] <asn:1> *
  CC      drivers/BiscuitOS/sparse.o
{% endhighlight %}

由于 src 指向了用户空间，所以 sparse 发出了警告。
