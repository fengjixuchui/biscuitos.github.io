...
__user 宏检查指针是否指向用户空间，如果指针不指向用户空间，那么
sparse 就是报错。__user 这个特性，即
__attribute__((noderef, address_space(1)))，是用来修饰一个变量的，
这个变量必须是非解除参考（no dereference）的，即这个变量地址必须是有
效的，而且变量所在的地址空间必须是 1，即用户程序空间的。这里把程序空间
分成了 3 个部分，0 表示 normal space，即普通地址空间，对内核代码来说，
...
实践代码中，由于将 src 指向了内核空间，那么 sparse 就会对这里报错。
