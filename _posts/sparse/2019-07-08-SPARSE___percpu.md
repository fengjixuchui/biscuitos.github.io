...
> [Github: __percpu](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/sparse/API/__percpu)
>
...
__percpu 宏用于限定变量或指针属于 percpu，如果将指针指向其他空间，如
内核空间，或则用户空间，那么 sparse 就会报错。
__attribute__((noderef, address_space(3)))，是用来修饰一个变量的，
这个变量必须是非解除参考（no dereference）的，即这个变量地址必须是有
效的，而且变量所在的地址空间必须是 3，即 percpu 空间的。这里把程序空间
分成了多个部分，0 表示 normal space，即普通地址空间，对内核代码来说，
当然就是内核空间地址了。1 表示用户地址空间，这个不用多讲，还有一个 3，
表示是 percpu 空间，预留给 percpu 变量的空间。
...
drivers/BiscuitOS/sparse.c:24:13:    expected unsigned int [noderef] <asn:3> *src
drivers/BiscuitOS/sparse.c:24:13:    got unsigned int *
  CC      drivers/BiscuitOS/sparse.o
{% endhighlight %}

如果将 percpu 指向其他空间，那么 sparse 就会报错。
