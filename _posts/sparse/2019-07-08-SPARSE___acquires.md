...
> [Github: __acquires](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/sparse/API/__acquires)
>
...
__acquires() 宏与锁有关，检查锁在使用前确保锁为 0，然后将其设置为 1.
__acquires() 与 __releases() 成对使用，用于检查在某个函数中，是否
正确的上锁和解锁操作。在实际使用中，__acquires(x) 和__releases(x) 必须成对
