---
layout: post
title:  "radix_tree_for_each_slot"
date:   2019-05-29 05:30:30 +0800
categories: [HW]
excerpt: Radix-Tree radix_tree_for_each_slot().
tags:
  - Radix-Tree
---
...
> [Github: radix_tree_for_each_slot](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/tree/radix-tree/API/radix_tree_for_each_slot)
>
...
radix_tree_for_each_slot() 函数用于遍历 radix tree 中所有的 slot。radix tree
采用了 struct radix_tree_iter 数据结构存储每次遍历到的数据，该结构中包含了节点
基本的信息。函数首先使用 for 循环进行遍历，首先调用 radix_tree_iter_init()
函数初始化了 struct radix_tree_iter 结构，接着开始遍历，每次遍历中，函数首先
判断 slot 是否存在，如果存在，就是用 slot 作为本次遍历的 iter；如果不存在，使用
radix_tree_next_chunk() 函数获得下一个 slot 对应的 struct radix_tree_iter
结构。函数每次遍历使用 radix_tree_next_slot() 函数，以此获得下一个 slot 对应
的 struct radix_tree_iter 结构。
