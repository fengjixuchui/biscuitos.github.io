...
rb_set_parent_color() 函数用于设置当前节点的父节点和颜色。在 rb_node 的定义中，
__rb_parent_color 成员用于同时存储父节点和颜色，颜色存储在 __rb_parent_color
的低 2 位，父节点则存储在 __rb_parent_color 但按 long 对齐，因此两个数据之间
不会相互影响。
...
static __init int rbtree_demo_init(void)
{
	...
	/* Set up parent and color */
	rb_set_parent_color(&node6.node, &node7.node, RB_RED);
