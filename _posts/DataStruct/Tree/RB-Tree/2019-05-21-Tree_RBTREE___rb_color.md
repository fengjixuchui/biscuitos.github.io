...
> [Github: __rb_color](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/tree/rb-tree/API/__rb_color)
>
...
__rb_color() 函数用于获得当前节点对应的颜色。首先可以查看 rb_node 结构的定义，如下：
...
rb_node 结构中，使用一个 long 变量 __rb_parent_color 同时存储 rb_node
的父节点和该节点的颜色，由于 struct rb_node 的结构是按 long 对齐，因此
struct rb_node 的每个成员的最低 2 位都是 0，因此将 __rb_parent_color
成员的低 2 为存储节点的颜色，剩余的位存储父节点的指针。因此在 __rb_color()
函数中，直接通过判断节点的 __rb_parent_color 的最低位来判断颜色，如果
最低位为 1，那么该节点是黑色；反之为红色。
...
static __init int rbtree_demo_init(void)
{
	...
	printk("%ld's color: %s", node7.runtime,
		__rb_color(node7.node.__rb_parent_color) ? "black" : "red");
