...
> [Github: rb_entry_safe](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/tree/rb-tree/API/rb_entry_safe)
>
...
rb_entry_safe() 函数用于安全的获得嵌套 rb_node 的数据结构。宏使用 typeof
定义了一个 ____ptr 用于存储 ptr，然后判断 ____ptr 是否存在，如果存在
才会去调用 rb_entry() 去获得嵌套 rb_node 的结构；如果不存在直接返回
NULL，这样就不会引起内核 panic。

> [rb_entry 源码分析](https://biscuitos.github.io/blog/Tree_RBTREE_rb_entry/#%E6%BA%90%E7%A0%81%E5%88%86%E6%9E%90)
...
static __init int rbtree_demo_init(void)
{
	...
	/* safe rb_entry */
	printk("Safe: %ld ",
		rb_entry_safe(&node6.node, struct node, node)->runtime);
