...
RB_EMPTY_NODE() 用于检查一个 rb_node 是否已经插入到一棵红黑树里面。
函数通过判断 rb_node 的 __rb_parent_color 成员的值进行判断。如果 __rb_parent_color
与自己相等，那么就说明该节点没有被插入到任何一棵红黑树里；反之则插入
到一棵红黑树里面。
...
RB_EMPTY_NODE() 函数用于判断 rb_node 是否插入到红黑树里。
