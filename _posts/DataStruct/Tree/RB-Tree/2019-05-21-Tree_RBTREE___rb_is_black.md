...
__rb_is_black() 函数判断当前 rb_node 的颜色是否是黑色。函数直接调用 __rb_color()
函数判断 rb_node 的颜色。如果 rb_node 的颜色是黑色，那么 __rb_is_black()
直接返回 1； 反之如果 rb_node 的颜色是红色，那么 __rb_is_black() 返回 0.

###### __rb_color

> [__rb_color 源码分析](https://biscuitos.github.io/blog/Tree_RBTREE___rb_color/)
...
static __init int rbtree_demo_init(void)
{
	...
	if (__rb_is_black(node7.node.__rb_parent_color))
