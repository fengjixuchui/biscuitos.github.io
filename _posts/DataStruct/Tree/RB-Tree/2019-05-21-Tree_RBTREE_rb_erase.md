...
> [Github: rb_erase](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/tree/rb-tree/API/rb_erase)
>
...
rb_erase() 函数用于从 rbtree 中移除一个 rb_node。函数调用 __rb_erase_augmented()
函数移除一个 rb_node 但未作红黑树的再次平衡，继续调用 ____rb_erase_color() 函数
再次平衡红黑树。关于红黑树删除节点的文档，请看：
...
移除红黑树的节点，rb_erase() 是内核提供的最上层的接口，方便删除一个红黑树节点。
