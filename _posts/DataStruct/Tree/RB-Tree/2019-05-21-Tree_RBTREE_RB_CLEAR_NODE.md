...
RB_CLEAR_NODE() 函数用于将一个 rb_node 从红黑树移除的后期, 以此安全移除节点。
函数用在红黑树移除的末期，红黑树节点与其他节点的关系已经处理完之后，使用
RB_CLEAR_NODE 函数进行最后的移除操作，将 __rb_parent_color 成员指向自己，
这样做是为了 RB_EMPTY_NODE() 函数便于判断一个红黑树节点是否在红黑树上。
...
static __init int rbtree_demo_init(void)
{
	...
	/* Remove rb_node from rbtree, no re-balance! */
	RB_CLEAR_NODE(&node5.node);
	/* Check rb_node whether insert in an rbtree  */
	if (RB_EMPTY_NODE(&node5.node))
		printk("%ld node deesn't insert RBTree.\n", node5.runtime);
...
RB_CLEAR_NODE 一般用于 rb_node 移除的后期。
