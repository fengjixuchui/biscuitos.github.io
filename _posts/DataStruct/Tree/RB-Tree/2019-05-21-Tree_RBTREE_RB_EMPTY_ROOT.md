...
> [Github: RB_EMPTY_ROOT](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/tree/rb-tree/API/RB_EMPTY_ROOT)
>
...
RB_EMPTY_ROOT() 函数用于检查当前 RBTree 是否为空。函数首先调用 READ_ONCE()
确保从内存中读取 root->rb_node 的最新值，然后判断这个新值是否等于 NULL。如果
红黑树为空，那么返回 1；反之返回 0.
...
static __init int rbtree_demo_init(void)
{
	...
	if (RB_EMPTY_ROOT(&BiscuitOS_rb))
		printk("RBTree: empty.. need add rb_node.\n");
	...
	if (!RB_EMPTY_ROOT(&BiscuitOS_rb))
		printk("RBTree: Tree isn't empty.\n");
...
{% highlight ruby %}
RBTree: empty.. need add rb_node.
RBTree: Tree isn't empty.
RB: 0x1
RB: 0x2
RB: 0x3
RB: 0x4
