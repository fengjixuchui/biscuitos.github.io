...
rb_first() 函数用于获得红黑树最左边的孩子，也就是最左边的叶子。rb_first()
通常用于红黑树遍历的时候，获得最左边的叶子，根据红黑树的定义，最左边的叶子就是
最小值的。函数首先获得红黑树根节点对应的 rb_node, 检查时候为空树，如果是直接
返回 NULL；反之使用 while 循环，找到最左边的节点，此时最左边的节点就是最小值的
节点。
