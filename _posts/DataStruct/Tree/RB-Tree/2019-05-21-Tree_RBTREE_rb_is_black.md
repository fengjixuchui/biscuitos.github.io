...
> [Github: rb_is_black](https://github.com/BiscuitOS/HardStack/tree/master/Algorithem/tree/rb-tree/API/rb_is_black)
>
...
rb_is_black() 函数判断当前节点是否是黑色。如果当前节点是黑色，那么直接返回 1；
如果不是，则返回 0. 函数直接调用 __rb_is_black() 函数，以及当前节点的
__rb_parent_color 成员，一起结合判断当前节点是否为黑色。

###### \_\_rb_is_black

> [\_\_rb_is_black 源码分析](https://biscuitos.github.io/blog/Tree_RBTREE___rb_is_red/#%E6%BA%90%E7%A0%81%E5%88%86%E6%9E%90)
...
static __init int rbtree_demo_init(void)
{
	...
	if (rb_is_black(&node7.node))
		printk("%ld's is black.\n", node7.runtime);
