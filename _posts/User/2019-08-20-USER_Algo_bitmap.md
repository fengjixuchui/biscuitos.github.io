...
##### <span id="A00">Algo bitmap 简介</span>

Algo bitmap 项目是将内核的 bitmap 框架移植到用户空间，提供接口给应用程序使用。
目前 BiscuitOS 已经支持 Algo bitmap 的移植和实践。开发者
可用通过下面的章节进行 Algo bitmap 的使用。
...
![](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/boot/BOOT000291.png)

此界面是 Package 支持软件的配置界面，开发者将光标移动到 "Algo - bitmap",
按下 "Y" 按键之后再按下回车键，进入 "Algo - bitmap" 配置界面。

![](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/boot/BOOT000292.png)

上图正是 "Algo bitmap" 应用程序的配置界面，"version" 选项代表当前软件的版本。
...
"bitmap-0.11", 进入该目录，可以获得两个文件: Makefile 和 README.md。至此
应用程序的移植前期准备已经结束。
...
进过上面的步骤之后，开发者在 "BiscuitOS/output/linux-5.0-arm32/package/bitmap-0.11"
目录下获得移植所需的 Makefile，然后开发者接下来需要做的就是下载源码，
使用如下命令：

{% highlight bash %}
cd BiscuitOS/output/linux-5.0-arm32/package/bitmap-0.11
make download
{% endhighlight %}

此时终端输出相关的信息，如下：

![](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/boot/BOOT000293.png)

此时在当前目录下会获得一个新的目录 "bitmap-0.11"，里面存储着源码相关的文件，
至此源码下载完毕。
...
![](https://raw.githubusercontent.com/EmulateSpace/PictureSet/master/BiscuitOS/boot/BOOT000294.png)
