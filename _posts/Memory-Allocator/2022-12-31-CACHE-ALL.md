...
> - [Linux CACHE 机制]()
>
>   - [IOREMAP MMIO CACHE Mechanism]()
>
>   - [MMIO PAT RBTree Mechanism]()
>
>   - [Page CACHE CPA Mechanism]()
>
>   - [Page Memory Type Mechanism]()
>
...
BiscuitOS 运行之后，加载 BiscuitOS-CACHE-UNCACHE-MMIO-KERNEL-default.ko 模块之后，从打印的内核信息可以看出模块已经映射了 MMIO，并且向 MMIO 写入 0x88520 之后可以读出正确的数据，最后查看 '/sys/kernel/debug/x86/pat_memtype_list' 节点查看系统 MMIO **memory_type** 信息，可以看到模块映射的 MMIO \[0xF0000000, 0xF0001000] 为 uncached, 另外查看 '/proc/iomem' 节点查看物理地址空间信息，可以看到 \[0xF0000000, 0xF0001000) 为 Broiler MMIO 区域, 实践符合预期. 接下来一个实践案例用于介绍如何将用户进程使用的内存缓存类型设置为 UC，BiscuitOS 部署逻辑如下:
...
make menuconfig
  ...
  [*] Package  --->
      ...
      [*] CACHE  --->
          [*] CACHE WT/WB/WC/WP/UC for Userspace Virtual Address(U)  --->
          -*- CACHE WT/WB/WC/WP/UC for Userspace Virtual Address(K)  --->

# 进入源码目录
# Userspace: BiscuitOS/output/linux-X.Y.Z-ARCH/package/BiscuitOS-CACHE-USER-PAGE-U-default/
# Kernel: BiscuitOS/output/linux-X.Y.Z-ARCH/package/BiscuitOS-CACHE-USER-PAGE-K-default/
cd BiscuitOS/output/linux-X.Y.Z-ARCH/package/BiscuitOS-CACHE-USER-PAGE-U-default/
# 部署源码
make prepare
make download
# 在 BiscuitOS 中实践
...
> [BiscuitOS-CACHE-USER-PAGE-U-default Source Code on Gitee](https://gitee.com/BiscuitOS_team/HardStack/tree/Gitee/Memory-Allocator/CACHE-MECH/BiscuitOS-CACHE-USER-PAGE)
...
BiscuitOS 在次运行之后，运行应用程序，可以看到系统没有再提示修改信息了，那么用户进程已经成功将一段物理内存的 **memory type** 设置为 UC. 另外通过前面的分析，如果将物理内存设置为 UC 之后，其性能将会受到极大影响，那么接下来通过一个实践案例对比 UC 和 WB(默认为 WB) 物理内存之间性能差异，案例在 BiscuitOS 的部署逻辑为:
...
**Strong Uncacheable(UC)**: Intel 架构下内存 Data Block 不会被缓存到 CACHE 中，CPU 所有的读写访问直接被发送到总线上. 这种 **memory type** 经常被用在映射 MMIO 的场景，但如果将普通内存的 **memory type** 也设置为 UC，那么会严重影响程序的性能. 本节用于介绍如何在内核空间将 MMIO 映射为 UC，其在 BiscuitOS 上的部署逻辑如下:
...
BiscuitOS 运行之后，加载 BiscuitOS-CACHE-UNCACHE-MMIO-KERNEL-default.ko 模块之后，从打印的内核信息可以看出模块已经映射了 MMIO，并且向 MMIO 写入 0x88520 之后可以读出正确的数据，最后查看 '/sys/kernel/debug/x86/pat_memtype_list' 节点查看系统 MMIO **memory_type** 信息，可以看到模块映射的 MMIO \[0xF0000000, 0xF0001000] 为 uncached, 另外查看 '/proc/iomem' 节点查看物理地址空间信息，可以看到 \[0xF0000000, 0xF0001000) 为 Broiler MMIO 区域, 实践符合预期.
...
**Uncacheable(UC-)**: Intel 架构下内存 Data Block 不会被缓存到 CACHE 中，CPU 所有的读写访问直接被发送到总线上. 这种类型在 Intel X86 下可以通过 PAT 机制设置，也会被 MTRR 机制覆盖修改。与 UC 类似，这种 **memory type** 经常被用在映射 MMIO 的场景, 但如果将普通内存的 **memory type** 也设置为 UC-，那么会严重影响程序的性能. 本节用于介绍如何在内核空间将 MMIO 映射为 UC-，其在 BiscuitOS 上的部署逻辑如下:
...
![](/assets/PDB/HK/TH002506.png)

源码通过一个内核模块进行展示，模块有两部分组成，第一部分是 20-35 行，模块向系统资源总线注册了一段 MMIO，这段 MMIO 起始物理地址是 BROILER_MMIO_BASE, 长度为 BROILER_MMIO_LEN, 注册完毕之后可以在系统启动之后，通过 '/proc/iomem' 接口查看到该段信息; 第二部分是 38-46 行，模块在 38 行调用 ioremap() 函数将 BROILER_MMIO_BASE 开始且长度为 BROILER_MMIO_LEN 的 MMIO 区域，映射为 Uncached- 的 **memory type**, 映射完毕之后函数在 46 行对 MMIO 进行访问，此时并不会触发缺页，因为 ioremap() 函数已为 MMIO 分配了对应的虚拟地址并建立了页表，因此模块可以直接访问 MMIO. 最后在 BiscuitOS 上实践模块，由于需要一段真实的 MMIO 才能运行模块，因此可使用 Broiler 模拟一段 MMIO，其起始物理地址为 0xF0000000(Broiler 已经模拟好硬件)，开发者直接在源码目录执行 **make broiler** 即可:

![](/assets/PDB/HK/TH002505.png)

BiscuitOS 运行之后，加载 BiscuitOS-CACHE-UNCACHED-MMIO-KERNEL-default.ko 模块之后，从打印的内核信息可以看出模块已经映射了 MMIO，并且向 MMIO 写入 0x88520 之后可以读出正确的数据，最后查看 '/sys/kernel/debug/x86/pat_memtype_list' 节点查看系统 MMIO **memory_type** 信息，可以看到模块映射的 MMIO \[0xF0000000, 0xF0001000] 为 uncached-minus, 另外查看 '/proc/iomem' 节点查看物理地址空间信息，可以看到 \[0xF0000000, 0xF0001000) 为 Broiler MMIO 区域, 实践符合预期.
...
**WriteCombining(WC)**: Intel 架构下内存 Data Block 不会被缓存到 CACHE 中，并且 CACHE 一致性不用保证，那么可以将写请求放入到 Write Combining Buffer 里，然后等待总线空闲时一并写入，这会导致写延时同时也减轻了总线压力. 这种类型在 Intel X86 下可以通过 PAT 机制进行选择，也会被 MTRR 机制进行设置。这种 **memory type** 经常被用在 Video Buffer、Graphics display, 这种场景的特点就是对写顺序不是很重要. 本节用于介绍如何在内核空间将 MMIO 映射为 WC，其在 BiscuitOS 上的部署逻辑如下:
...
> [BiscuitOS-CACHE-WC-MMIO-KERNEL-default Source Code on Gitee](https://gitee.com/BiscuitOS_team/HardStack/tree/Gitee/Memory-Allocator/CACHE-MECH/BiscuitOS-CACHE-WC-MMIO-KERNEL)

![](/assets/PDB/HK/TH002507.png)
...
![](/assets/PDB/HK/TH002508.png)

BiscuitOS 运行之后，加载 BiscuitOS-CACHE-WC-MMIO-KERNEL-default.ko 模块之后，从打印的内核信息可以看出模块已经映射了 MMIO，并且向 MMIO 写入 0x88520 之后可以读出正确的数据，最后查看 '/sys/kernel/debug/x86/pat_memtype_list' 节点查看系统 MMIO **memory_type** 信息，可以看到模块映射的 MMIO \[0xF0000000, 0xF0001000] 为 write-combining, 另外查看 '/proc/iomem' 节点查看物理地址空间信息，可以看到 \[0xF0000000, 0xF0001000) 为 Broiler MMIO 区域, 实践符合预期.
...
**WriteThrough(WT)**: Intel 架构下内存 Data Block 被缓存到 CACHE 中，其与 WriteBack 类似，但不同点在于当发生写时 CACHE Hit, 那么 WT 不仅写入 CACHE Line，还会写入到内存里，但如果 CACHE Miss，那么 WT 仅仅写入内存. 这种 **memory type** 经常被用在 Frame Buffer 和直接访问系统内存的设备上，这里设备不具有监听内存访问能力的. 本节用于介绍如何在内核空间将 MMIO 映射为 WT，其在 BiscuitOS 上的部署逻辑如下:
...
> [BiscuitOS-CACHE-WT-MMIO-KERNEL-default Source Code on Gitee](https://gitee.com/BiscuitOS_team/HardStack/tree/Gitee/Memory-Allocator/CACHE-MECH/BiscuitOS-CACHE-WT-MMIO-KERNEL)

![](/assets/PDB/HK/TH002509.png)

源码通过一个内核模块进行展示，模块有两部分组成，第一部分是 20-35 行，模块向系统资源总线注册了一段 MMIO，这段 MMIO 起始物理地址是 BROILER_MMIO_BASE, 长度为 BROILER_MMIO_LEN, 注册完毕之后可以在系统启动之后，通过 '/proc/iomem' 接口查看到该段信息; 第二部分是 38-46 行，模块在 38 行调用 ioremap_wt() 函数将 BROILER_MMIO_BASE 开始且长度为 BROILER_MMIO_LEN 的 MMIO 区域，映射为 WriteThrough 的 **memory type**, 映射完毕之后函数在 46 行对 MMIO 进行访问，此时并不会触发缺页，因为 ioremap_wt() 函数已为 MMIO 分配了对应的虚拟地址并建立了页表，因此模块可以直接访问 MMIO. 最后在 BiscuitOS 上实践模块，由于需要一段真实的 MMIO 才能运行模块，因此可使用 Broiler 模拟一段 MMIO，其起始物理地址为 0xF0000000(Broiler 已经模拟好硬件)，开发者直接在源码目录执行 **make broiler** 即可:

![](/assets/PDB/HK/TH002510.png)

BiscuitOS 运行之后，加载 BiscuitOS-CACHE-WT-MMIO-KERNEL-default.ko 模块之后，从打印的内核信息可以看出模块已经映射了 MMIO，并且向 MMIO 写入 0x88520 之后可以读出正确的数据，最后查看 '/sys/kernel/debug/x86/pat_memtype_list' 节点查看系统 MMIO **memory_type** 信息，可以看到模块映射的 MMIO \[0xF0000000, 0xF0001000] 为 write-through, 另外查看 '/proc/iomem' 节点查看物理地址空间信息，可以看到 \[0xF0000000, 0xF0001000) 为 Broiler MMIO 区域, 实践符合预期.
...
##### UC- MMIO on Userspace

**Uncacheable(UC-)**: Intel 架构下内存 Data Block 不会被缓存到 CACHE 中，CPU 所有的读写访问直接被发送到总线上. 这种类型在 Intel X86 下可以通过 PAT 机制设置，也会被 MTRR 机制覆盖修改。与 UC 类似，这种 **memory type** 经常被用在映射 MMIO 的场景, 但如果将普通内存的 **memory type** 也设置为 UC-，那么会严重影响程序的性能. 本节用于介绍如何在用户空间将 MMIO 映射为 UC-，其在 BiscuitOS 上的部署逻辑如下:
...
![](/assets/PDB/HK/TH002511.png)

案例有两个程序组成，其中一个为内核模块，内核模块的实现如上图，其主要功能是在系统资源总线上添加 MMIO 区域 \[0xF0000000, 0xF0001000)，只有系统物理地址空间资源树上有 MMIO 区域之后，用户空间的程序才可以将 MMIO 映射到用户进程的地址空间，然后进行访问.

![](/assets/PDB/HK/TH002512.png)

案例的另外一个程序则是一个用户空间程序，程序目的是将指定的 MMIO 映射到自己的地址空间，然后进行访问. 具体逻辑是程序在 29 行通过打开 "/dev/mem" 节点，然后在 36 行通过 mmap() 函数将 MMIO_BASE 起始且长度为 MMIO_LEN 的区域映射到进程的地址空间，程序接着在 50 行访问 MMIO，最后在 57-58 行解除对 MMIO 的映射，最后关闭 "/dev/mem" 节点. 以上便是用户空间访问 MMIO 的过程, 接下来是在 BiscuitOS 上实践，由于需要一段真实的 MMIO 才能运行模块，因此可使用 Broiler 模拟一段 MMIO，其起始物理地址为 0xF0000000(Broiler 已经模拟好硬件)，开发者直接在源码目录执行 **make broiler** 即可:

![](/assets/PDB/HK/TH002513.png)

BiscuitOS 运行之后，加载 BiscuitOS-CACHE-UNCACHED-MMIO-USERSPACE-default.ko 模块之后，然后压后台方式运行 APP 应用程序，从打印信息可以看出应用程序向 MMIO 写入 0x88520 之后可以读出正确的数据，最后查看 '/sys/kernel/debug/x86/pat_memtype_list' 节点查看系统 MMIO **memory_type** 信息，可以看到应用程序映射的 MMIO \[0xF0000000, 0xF0001000] 为 uncached-minus, 另外查看 '/proc/iomem' 节点查看物理地址空间信息，可以看到 \[0xF0000000, 0xF0001000) 为 Broiler MMIO 区域, 实践符合预期.
...
**Uncacheable(UC-)**: Intel 架构下内存 Data Block 不会被缓存到 CACHE 中，CPU 所有的读写访问直接被发送到总线上. 这种类型在 Intel X86 下可以通过 PAT 机制设置，也会被 MTRR 机制覆盖修改。与 UC 类似，这种 **memory type** 经常被用在映射 MMIO 的场景, 但如果将普通内存的 **memory type** 也设置为 UC-，那么会严重影响程序的性能. 本节用于介绍如何在内核空间将普通内存映射为 UC-，其在 BiscuitOS 上的部署逻辑如下:
...
![](/assets/PDB/HK/TH002514.png)

源码通过一个内核模块进行展示，模块首先在 21 行通过 \_\_get_free_page() 函数从 Buddy 分配器中分配一个物理页，并将物理页在内核空间的虚拟地址存储在 addr 变量中，模块接着在 28 行调用 set_memory_uc() 函数和传入 addr 变量，将对应的虚拟内存的 CACHE mode 设置为 **Uncache minus(UC-)**, ，模块接下来在 30 行访问对应的虚拟内存，最后在 36 行调用 free_page() 函数释放虚拟内存并将物理页归还 Buddy 分配器. 最后在 BiscuitOS 上实践模块:

![](/assets/PDB/HK/TH002515.png)

BiscuitOS 运行之后，首先查看 "/sys/kernel/debug/x86/cpa_stats" 节点信息，该节点记录了普通内存页 CACHE mode 变化计数，接着加载 BiscuitOS-CACHE-UNCACHED-MEM-KERNEL-default.ko 模块之后，从打印的内核信息可以看出，模块已经修改了物理页对应的虚拟内存映射 CACHE mode，最后再次查看 "/sys/kernel/debug/x86/cpa_stats" 节点信息，可以看到 "4K pages set-checked" 数量有改变，实践符合预期.
